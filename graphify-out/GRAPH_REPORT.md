# Graph Report - Anamnesis  (2026-08-31)

## Corpus Check
- 148 files · ~455,362 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2832 nodes · 4018 edges · 354 communities (187 shown, 167 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 89 edges (avg confidence: 0.81)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4093910d`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- Anamnesis.slnx
- Projectile
- @PlayerControls
- com.unity.mathematics
- TMP_FrameRateCounter
- com.unity.render-pipelines.core
- EntityProjectileHandler
- RewardButton
- WaveManager
- Anamnesis
- HoverScale
- Changelog
- com.unity.test-framework
- com.unity.modules.imgui
- GearItem
- .ApplyKnockback
- Rare Pool
- EnemyAttackHandler
- TMP_TextEventHandler
- dependencies
- bool
- Player Upgrades
- PlayerSkillTree
- TMP_TextInfoDebugTool
- TMP_TextSelector_B
- CrystalFlux.StatusEffectSystem
- PlayerUpgrade
- com.unity.modules.uielements
- EnvMapAnimator
- ChatController
- TypeSelectorDrawer
- AnomalyInstance
- AdditionalDamage
- EntityHealth
- CrystalFlux.WaveSystem
- SkillNodeUI
- CrystalFlux.Core
- SkillTreePanZoom
- TMPro.Examples
- com.unity.2d.tilemap
- SkillTreeUI
- UnlimitedWaveManager
- TextMeshProFloatingText
- EntitySummonHandler
- EnemyMovement
- SkewTextExample
- TMPro
- SoulRend
- EntityStatManager
- GAME.md
- SkillTreeLineRenderer
- VertexJitter
- dependencies
- BossBarUI
- Item
- TooltipTrigger
- CameraController
- .GetCurrentWave
- PlayerUI
- com.unity.modules.physics
- README.md
- .HandleHitEntity
- SkillNodeDef
- VertexShakeA
- TextConsoleSimulator
- VertexZoom
- TODO.md
- VertexShakeB
- com.crystalflux.core
- com.unity.ext.nunit
- PlayerResourcePool
- CooldownAdvance.cs
- EntitySplitting
- Benchmark01
- Benchmark01_UGUI
- TMP_TextSelector_A
- [v0.3.3_2] - 2026-08-30
- com.unity.ugui
- com.unity.2d.common
- ObjectSpin
- TMP_TextEventCheck
- com.unity.modules.imageconversion
- ⚠️ [v0.2.13] - 2026-08-27
- StatusEffectManager
- AttackAsset
- AttackReplacement
- ShaderPropAnimator
- [v0.3.3] - 2026-08-30 — Projectile Movement Patterns & Screen-Wide Spawn Lines
- com.unity.modules.ui
- StatBuffs
- [v0.1.11] - 2026-08-24 - Currency Update
- .ResumeGameLoop
- [v0.1.10_1] - 2026-08-23
- .TriggerUpgrades
- ProjectileData
- ProjectileDamageSnapshot
- Updated
- com.unity.modules.physics2d
- com.unity.modules.unitywebrequestaudio
- EnemyStatManager
- bool
- .WaveSpawnRoutine
- SimpleScript
- PlayerLevel
- Workflow
- TeleType
- [v0.3.1] - 2026-08-29 — Compiler-Enforced Assembly Boundaries
- .OnMilestoneRewardClaimed
- TextIndicatorSpawner
- [v0.0.22] - 2026-08-06
- Paradox
- [v0.1.5] - 2026-08-10
- ⚠️ [v0.2.9] - 2026-08-26 - Stat System Refactor
- [v0.1.10_2] - 2026-08-23
- StellarSurge
- TMP_ExampleScript_01
- PlayerUpgradeManager
- Color
- [v0.2.7] - 2026-08-26 - Enemy Scaling Overhaul
- WarpTextExample
- [v0.2.4] - 2026-08-25 - Gamemode Selector Update
- [v0.2.6] - 2026-08-26 - Skill Tree Enhancement Update
- [v0.1.12] - 2026-08-24 - Milestone Rewards Update
- DropdownSample
- [v0.0.13] - 2026-07-31
- TooltipUI
- [v0.0.9_1] - 2026-07-29
- [v0.1.6] - 2026-08-13 - The Titles Update
- [v0.1.2] - 2026-08-10
- [v0.1.9_1] - 2026-08-23
- Dictionary
- SkillTreeInputToggle
- [v0.2.2] - 2026-08-25 - Skill Tree Expansion Update
- EntityGearManager
- UnityEvent
- EnemyPhase
- [v0.3.5] - 2026-08-30 - Content Expansion Update
- [v0.2.5] - 2026-08-25
- [v0.0.23] - 2026-08-06
- MonoBehaviour
- PlayerMovement
- ⚠️ [v0.2.16] - 2026-08-27
- ⚠️ [v0.2.12] - 2026-08-26
- [v0.3.4] - 2026-08-30
- [v0.1.3] - 2026-08-10
- com.unity.modules.audio
- .BuildDamagePacket
- Decoy
- UnlockEffect
- ⚠️ [v0.2.8_2] - 2026-08-26
- WaveSequence
- com.crystalflux.core
- com.unity.2d.animation
- com.unity.2d.aseprite
- ⚠️ [v0.2.14] - 2026-08-27
- com.unity.2d.psdimporter
- [v0.1.12_1] - 2026-08-24
- com.unity.2d.spriteshape
- [v0.3.9] - 2026-08-31
- [v0.3.8_1] - 2026-08-31
- com.unity.2d.tilemap.extras
- com.unity.2d.tooling
- com.unity.cinemachine
- com.unity.collab-proxy
- [v0.2.1] - 2026-08-25
- com.unity.collab-proxy
- com.unity.ide.rider
- com.unity.inputsystem
- com.unity.modules.androidjni
- [v0.3.6] - 2026-08-30
- com.unity.modules.cloth
- [v0.1.0-PR2] - 2026-08-09
- copilot-instructions.md
- StatReduction
- Pulled
- [v0.1.13_1] - 2026-08-24
- [v0.3.8] - 2026-08-31
- [v0.0.23_3] - 2026-08-06
- com.unity.modules.unitywebrequesttexture
- com.unity.modules.unitywebrequestwww
- com.unity.modules.vehicles
- com.unity.modules.video
- com.unity.modules.wind
- com.unity.multiplayer.center
- com.unity.render-pipelines.universal
- com.unity.2d.sprite
- [v0.0.21] - 2026-08-05 — Knockback Update Part 2
- GainMana
- ⚠️ [v0.2.11] - 2026-08-26
- [v0.0.19] - 2026-08-05
- [v0.3.3_1] - 2026-08-30
- com.unity.2d.tilemap
- com.unity.modules.vectorgraphics
- com.unity.modules.animation
- com.unity.modules.assetbundle
- com.unity.modules.imgui
- com.unity.modules.jsonserialize
- com.unity.modules.particlesystem
- com.unity.modules.physics
- com.unity.modules.tilemap
- com.unity.modules.ui
- com.unity.modules.uielements
- com.unity.modules.xr
- com.unity.test-framework
- com.unity.ugui
- com.unity.modules.audio
- TextIndicator
- EntityStats
- .AddStat
- [v0.1.8_5] - 2026-08-22
- [v0.1.0-PR1] - 2026-08-09 — Skill Tree Update
- com.unity.modules.terrain
- .ChangeHealth
- [v0.2.4_1] - 2026-08-25
- [v0.2.0] - 2026-08-24 — Progression, Economy & Milestones Update (Release Summary)
- com.unity.modules.unitywebrequest
- .SpawnTextIndicator
- [v0.0.10] - 2026-07-30
- [v0.2.2_1] - 2026-08-25
- ⚠️ [v0.2.15] - 2024-08-27
- [v0.1.13] - 2026-08-24 - Stats Extension Update
- [v0.0.14] - 2026-08-03
- CrystalFlux.SkillTree
- Color
- [v0.1.11_1] - 2026-08-24
- float
- [v0.0.11] - 2026-07-30
- [v0.0.1_1] - 2026-07-29
- GameObject
- .FindClosestEnemyInDirection
- VertexColorCycler
- bool
- Color
- float
- [v0.0.9_2] - 2026-07-29
- GameObject
- List
- Vector2
- GameObject
- Vector3
- StatBuff
- GameObject
- PlayerAttackCooldownUI
- List
- Vector2
- PlayerDashCooldownUI
- Vector2
- .HandlePatternMovement
- Vector2
- GameObject
- GameObject
- [v0.0.9] - 2026-07-29
- Reminiscence
- [v0.0.1] - 2026-07-29 — Initial Release
- [v0.2.8] - 2026-08-26
- Added
- [v0.1.9_2] - 2026-08-23
- SpawnProjectile
- IOrbitRegister
- [v0.3.2] - 2026-08-29 — Core Extracted to a Package, Wave Decoupled
- ⚠️ [v0.2.18] - 2026-08-28
- PropertyAttribute
- [v0.1.6_1] - 2026-08-15
- [v0.1.0-PR3_1] - 2026-08-09
- [v0.3.7] - 2026-08-31
- [v0.0.16_2] - 2026-08-05 — Dash Update Part 2
- [v0.2.19] - 2026-08-29
- [v0.1.7_1] - 2026-08-18
- Animator
- [v0.1.0-PR3] - 2026-08-09
- Canvas
- [v0.0.7_1] - 2026-07-29
- [v0.1.8_4] - 2026-08-22
- .StartDeathSequence
- Coroutine
- DamagePacket
- Array
- IEnumerator
- Quaternion
- TMP_FontAsset
- AttackData
- EffectAsset
- bool
- float
- bool
- Color
- float
- float
- Vector2
- float
- StatBuff
- List
- float
- StatBuff
- WaitForSeconds
- IStatProvider
- DamageType
- GameObject
- IEnumerator
- IReadOnlyList
- Slider
- GrantStatusEffect
- [v0.3.0] - 2026-08-29 — System Refactor & QoL Update (Release Summary)
- Color
- RectTransform
- SkillNodeDef
- SkillTreeManager
- [v0.1.8] - 2026-08-18 - Content Update v3 - Part II
- Tooltip
- Vector3
- [v0.0.2] - 2026-07-29
- AttackReward
- DamageCalculator
- [v0.1.10_3] - 2026-08-24
- GeneratedReward
- IAnnouncer
- GameController
- IAttackHandler
- ICurrencyHolder
- ISkillPointHolder
- IUpgradeHolder
- string
- MilestoneRewardData
- com.unity.ide.visualstudio
- PlayerUpgradeReward
- StatType
- EffectData
- TextMeshProUGUI
- hp
- [v0.3.2_1] - 2026-08-29
- ProjectileDamageSnapshot
- ProjectileData
- Rigidbody2D
- AnomalyInstance
- Camera
- AttackAsset
- AttackType
- Sprite
- com.unity.modules.director
- StatBuff
- UpgradeAsset
- Transform
- com.unity.modules.screencapture
- Vector2
- List
- com.unity.modules.jsonserialize
- com.unity.modules.umbra
- com.unity.modules.unitywebrequest
- com.unity.modules.vr
- com.unity.visualscripting
- com.unity.modules.terrainphysics
- com.unity.timeline

## God Nodes (most connected - your core abstractions)
1. `Changelog` - 113 edges
2. `WaveManager` - 89 edges
3. `Projectile` - 54 edges
4. `CrystalFlux.Core` - 54 edges
5. `AttackData` - 42 edges
6. `EntityHealth` - 37 edges
7. `PlayerAttackHandler` - 34 edges
8. `PlayerSkillTree` - 32 edges
9. `CrystalFlux.EntitySystem` - 30 edges
10. `PlayerUpgradeManager` - 29 edges

## Surprising Connections (you probably didn't know these)
- `UnlimitedWaveButtonController` --references--> `UnlimitedWaveManager`  [EXTRACTED]
  Assets/scripts/Wave/UnlimitedWaveButtonController.cs → Assets/scripts/Wave/UnlimitedWaveManager.cs
- `UnlimitedWaveManager` --inherits--> `WaveManager`  [EXTRACTED]
  Assets/scripts/Wave/UnlimitedWaveManager.cs → Assets/scripts/Wave/WaveManager.cs
- `RegularWaveButtonController` --references--> `WaveManager`  [EXTRACTED]
  Assets/scripts/Wave/RegularWaveButtonController.cs → Assets/scripts/Wave/WaveManager.cs
- `Benchmark02` --references--> `TextMeshProFloatingText`  [EXTRACTED]
  Assets/TextMesh Pro/Examples & Extras/Scripts/Benchmark02.cs → Assets/TextMesh Pro/Examples & Extras/Scripts/TextMeshProFloatingText.cs
- `PlayerLevel` --references--> `PlayerUpgradeManager`  [EXTRACTED]
  Assets/scripts/Entity/Player/PlayerLevel.cs → Assets/scripts/Entity/Player/PlayerUpgradeManager.cs

## Import Cycles
- None detected.

## Communities (354 total, 167 thin omitted)

### Community 0 - "Anamnesis.slnx"
Cohesion: 0.02
Nodes (98): Assembly-CSharp, Assembly-CSharp-Editor, CrystalFlux.Entity, CrystalFlux.Projectile, CrystalFlux.SkillTree, CrystalFlux.StatusEffect, CrystalFlux.TextIndicator, CrystalFlux.Wave (+90 more)

### Community 1 - "Projectile"
Cohesion: 0.11
Nodes (10): Projectile, bool, Camera, float, int, List, MovementType, ProjectileDamageSnapshot (+2 more)

### Community 2 - "@PlayerControls"
Cohesion: 0.06
Nodes (22): CallbackContext, IEnumerator, List, IPlayerActions, IUIActions, PlayerActions, @PlayerControls, UIActions (+14 more)

### Community 3 - "com.unity.mathematics"
Cohesion: 0.10
Nodes (20): com.unity.burst, com.unity.mathematics, dependencies, depth, source, url, version, dependencies (+12 more)

### Community 4 - "TMP_FrameRateCounter"
Cohesion: 0.07
Nodes (23): Camera, float, int, string, TextMeshPro, Transform, FpsCounterAnchorPositions, TMP_FrameRateCounter (+15 more)

### Community 5 - "com.unity.render-pipelines.core"
Cohesion: 0.07
Nodes (30): com.unity.render-pipelines.core, com.unity.render-pipelines.universal-config, com.unity.searcher, com.unity.shadergraph, dependencies, depth, source, version (+22 more)

### Community 6 - "EntityProjectileHandler"
Cohesion: 0.16
Nodes (8): EntityProjectileHandler, Camera, int, List, Transform, Vector2, Vector3, IOrbitRegister

### Community 7 - "RewardButton"
Cohesion: 0.09
Nodes (31): RewardButton, RewardType, Action, AttackReward, bool, GeneratedReward, Image, MilestoneRewardData (+23 more)

### Community 8 - "WaveManager"
Cohesion: 0.09
Nodes (14): WaveManager, AnomalyInstance, bool, float, GameObject, int, Transform, BaseReward (+6 more)

### Community 9 - "Anamnesis"
Cohesion: 0.13
Nodes (15): About, Anamnesis, Assembly Boundaries, Awakening trigger conditions, Changelog, Content, Controls, Core Loop (+7 more)

### Community 10 - "HoverScale"
Cohesion: 0.13
Nodes (7): HoverScale, bool, float, Transform, CrystalFlux.Utils, PointerEventData, Vector3

### Community 11 - "Changelog"
Cohesion: 0.03
Nodes (71): Added, Added, Added, Added, Added, Added, Added, Added (+63 more)

### Community 12 - "com.unity.test-framework"
Cohesion: 0.08
Nodes (26): com.unity.nuget.mono-cecil, com.unity.test-framework.performance, dependencies, dependencies, depth, source, url, version (+18 more)

### Community 13 - "com.unity.modules.imgui"
Cohesion: 0.08
Nodes (26): com.unity.settings-manager, com.unity.splines, dependencies, depth, source, url, version, dependencies (+18 more)

### Community 14 - "GearItem"
Cohesion: 0.31
Nodes (8): EquipmentSlot, GearItem, StatRoll, StatRollType, float, int, List, StatBuff

### Community 15 - ".ApplyKnockback"
Cohesion: 0.33
Nodes (6): AppliedForce, AppliedForce, KnockbackHandler, float, List, Vector2

### Community 16 - "Rare Pool"
Cohesion: 0.07
Nodes (28): Aphelion, Astral Nova, Blaze, Blaze Hyperspark, Blaze Spark, Blood Pact, Cosmic Blaze, Exodus (+20 more)

### Community 17 - "EnemyAttackHandler"
Cohesion: 0.12
Nodes (11): EnemyAttackHandler, Animator, bool, float, GameObject, HashSet, IEnumerator, int (+3 more)

### Community 18 - "TMP_TextEventHandler"
Cohesion: 0.13
Nodes (11): Camera, Canvas, int, PointerEventData, TMP_Text, TMP_TextEventHandler, CharacterSelectionEvent, LineSelectionEvent (+3 more)

### Community 19 - "dependencies"
Cohesion: 0.07
Nodes (26): dependencies, depth, source, version, dependencies, depth, source, version (+18 more)

### Community 21 - "Player Upgrades"
Cohesion: 0.14
Nodes (14): Autopilot, Decoy, Decoy Upgraded (Capstone), Feedback Loop, Hex Cast, Hypercarry, Paradox, Player Upgrades (+6 more)

### Community 22 - "PlayerSkillTree"
Cohesion: 0.12
Nodes (11): PlayerSkillTree, bool, canUnlock, Dictionary, failMessage, HashSet, List, SkillNodeDef (+3 more)

### Community 23 - "TMP_TextInfoDebugTool"
Cohesion: 0.20
Nodes (9): bool, Color, float, string, TMP_Text, Transform, Vector3, TMP_TextInfoDebugTool (+1 more)

### Community 24 - "TMP_TextSelector_B"
Cohesion: 0.10
Nodes (13): bool, Camera, Canvas, int, Object, PointerEventData, RectTransform, string (+5 more)

### Community 25 - "CrystalFlux.StatusEffectSystem"
Cohesion: 0.09
Nodes (14): Detonator, Color, DamageType, float, DoT, bool, Color, DamageType (+6 more)

### Community 26 - "PlayerUpgrade"
Cohesion: 0.15
Nodes (9): PlayerUpgrade, TriggerCondition, float, GameObject, List, Vector2, UpgradeAsset, HexCast (+1 more)

### Community 27 - "com.unity.modules.uielements"
Cohesion: 0.09
Nodes (22): dependencies, depth, source, url, version, dependencies, depth, source (+14 more)

### Community 28 - "EnvMapAnimator"
Cohesion: 0.25
Nodes (5): IEnumerator, Material, TMP_Text, Vector3, EnvMapAnimator

### Community 29 - "ChatController"
Cohesion: 0.25
Nodes (4): ChatController, TMP_Text, Scrollbar, TMP_InputField

### Community 30 - "TypeSelectorDrawer"
Cohesion: 0.21
Nodes (9): TypeSelectorDrawer, List, string, CrystalFlux.EditorTools, GUIContent, PropertyDrawer, Rect, SerializedProperty (+1 more)

### Community 31 - "AnomalyInstance"
Cohesion: 0.06
Nodes (19): AnomalyButtonUI, Action, TextMeshProUGUI, AnomalyData, AnomalyType, float, int, string (+11 more)

### Community 32 - "AdditionalDamage"
Cohesion: 0.40
Nodes (4): AdditionalDamage, DamageType, float, GameObject

### Community 33 - "EntityHealth"
Cohesion: 0.12
Nodes (14): Animator, EntityHealth, bool, Camera, float, int, IStatProvider, TextMeshProUGUI (+6 more)

### Community 34 - "CrystalFlux.WaveSystem"
Cohesion: 0.11
Nodes (6): RegularWaveButtonController, UnlimitedWaveButtonController, WaveQuality, List, CrystalFlux.Core, CrystalFlux.WaveSystem

### Community 35 - "SkillNodeUI"
Cohesion: 0.11
Nodes (15): SkillNodeUI, GameObject, Image, offset, PointerEventData, SkillNodeDef, subtitle, title (+7 more)

### Community 36 - "CrystalFlux.Core"
Cohesion: 0.14
Nodes (5): CrystalFlux.EntitySystem, CrystalFlux.ProjectileSystem, CrystalFlux.UISystem, CrystalFlux.Core, UnityEngine.UI

### Community 37 - "SkillTreePanZoom"
Cohesion: 0.19
Nodes (5): SkillTreePanZoom, bool, float, RectTransform, Vector2

### Community 38 - "TMPro.Examples"
Cohesion: 0.13
Nodes (7): bool, int, Benchmark02, Font, int, TextMeshSpawner, TMPro.Examples

### Community 39 - "com.unity.2d.tilemap"
Cohesion: 0.09
Nodes (24): dependencies, depth, dependencies, depth, source, url, version, source (+16 more)

### Community 40 - "SkillTreeUI"
Cohesion: 0.20
Nodes (8): SkillTreeUI, bool, Dictionary, float, GameObject, IReadOnlyList, RectTransform, SkillNodeDef

### Community 41 - "UnlimitedWaveManager"
Cohesion: 0.17
Nodes (9): EnemySpawnInfo, UnlimitedWaveManager, bool, float, GameObject, IEnumerator, int, List (+1 more)

### Community 42 - "TextMeshProFloatingText"
Cohesion: 0.14
Nodes (13): bool, Font, GameObject, IEnumerator, int, Quaternion, TextMesh, TextMeshPro (+5 more)

### Community 43 - "EntitySummonHandler"
Cohesion: 0.19
Nodes (10): EntitySummonHandler, bool, float, GameObject, IEnumerator, int, List, Vector2 (+2 more)

### Community 44 - "EnemyMovement"
Cohesion: 0.12
Nodes (13): EnemyMovement, Animator, bool, float, GameObject, int, IReadOnlyList, IStatProvider (+5 more)

### Community 45 - "SkewTextExample"
Cohesion: 0.27
Nodes (5): AnimationCurve, float, IEnumerator, TMP_Text, SkewTextExample

### Community 46 - "TMPro"
Cohesion: 0.20
Nodes (4): TMP_DigitValidator, TMP_PhoneNumberValidator, TMPro, TMP_InputValidator

### Community 47 - "SoulRend"
Cohesion: 0.14
Nodes (6): EffectData, GameObject, SoulRendPU, SoulRend, float, StatBuff

### Community 48 - "EntityStatManager"
Cohesion: 0.14
Nodes (8): EntityStatManager, Dictionary, HashSet, int, List, ICurrencyHolder, IStatProvider, ITeamMember

### Community 49 - "GAME.md"
Cohesion: 0.13
Nodes (14): Autopilot, Cyclone Cleave, Decoy Burst, Feedback Loop, Game Data Reference, Lacerate, Skill Tree Attacks, Soul Fragment (+6 more)

### Community 50 - "SkillTreeLineRenderer"
Cohesion: 0.15
Nodes (12): SkillTreeLineRenderer, Color, Dictionary, float, HashSet, Image, int, IReadOnlyList (+4 more)

### Community 51 - "VertexJitter"
Cohesion: 0.16
Nodes (7): bool, float, IEnumerator, Object, TMP_Text, VertexAnim, VertexJitter

### Community 52 - "dependencies"
Cohesion: 0.14
Nodes (13): com.unity.modules.accessibility, com.unity.modules.adaptiveperformance, com.unity.modules.ai, com.unity.modules.unityanalytics, dependencies, com.unity.modules.accessibility, com.unity.modules.adaptiveperformance, com.unity.modules.ai (+5 more)

### Community 53 - "BossBarUI"
Cohesion: 0.25
Nodes (7): BossBarUI, Image, int, IStatProvider, Slider, TextMeshProUGUI, IBossBar

### Community 54 - "Item"
Cohesion: 0.33
Nodes (5): Item, ItemRarity, Sprite, string, CrystalFlux.ItemSystem

### Community 55 - "TooltipTrigger"
Cohesion: 0.24
Nodes (6): TooltipTrigger, PointerEventData, string, Vector2, IPointerExitHandler, ITooltipDisplay

### Community 56 - "CameraController"
Cohesion: 0.17
Nodes (8): bool, float, string, Transform, Vector3, CameraController, CameraModes, CameraModes

### Community 58 - "PlayerUI"
Cohesion: 0.12
Nodes (16): PlayerResourceUI, GameObject, ICurrencyHolder, IStatProvider, offset, PointerEventData, subtitle, title (+8 more)

### Community 59 - "com.unity.modules.physics"
Cohesion: 0.07
Nodes (27): dependencies, depth, source, version, dependencies, depth, source, version (+19 more)

### Community 60 - "README.md"
Cohesion: 0.20
Nodes (6): Credits, Pre [v0.4.0], Roadmap, [v0.1.0] - Skill Tree Update (Released), [v0.2.0] - Content Expansion Update (Released), [v0.3.0] - System Refactor & QoL Update

### Community 61 - ".HandleHitEntity"
Cohesion: 0.17
Nodes (8): GameObject, IEnumerator, AttackData, Collider2D, EffectData, hp, mana, stamina

### Community 62 - "SkillNodeDef"
Cohesion: 0.18
Nodes (9): SkillNodeDef, bool, int, List, Sprite, string, SkillTreeDefinition, List (+1 more)

### Community 63 - "VertexShakeA"
Cohesion: 0.17
Nodes (6): bool, float, IEnumerator, Object, TMP_Text, VertexShakeA

### Community 64 - "TextConsoleSimulator"
Cohesion: 0.21
Nodes (5): bool, IEnumerator, Object, TMP_Text, TextConsoleSimulator

### Community 65 - "VertexZoom"
Cohesion: 0.17
Nodes (6): bool, float, IEnumerator, Object, TMP_Text, VertexZoom

### Community 66 - "TODO.md"
Cohesion: 0.22
Nodes (8): Content Updates, Major, Minor, Notes, Planned Features, Pre [v0.4.0] Checklist, Pre [v0.5.0] Checklist, QoL Updates

### Community 67 - "VertexShakeB"
Cohesion: 0.17
Nodes (6): bool, float, IEnumerator, Object, TMP_Text, VertexShakeB

### Community 68 - "com.crystalflux.core"
Cohesion: 0.33
Nodes (6): dependencies, depth, hash, source, version, com.crystalflux.core

### Community 69 - "com.unity.ext.nunit"
Cohesion: 0.15
Nodes (13): com.unity.ext.nunit, dependencies, depth, source, version, dependencies, depth, source (+5 more)

### Community 70 - "PlayerResourcePool"
Cohesion: 0.24
Nodes (6): PlayerResourcePool, bool, float, IStatProvider, IResourcePool, ResourceType

### Community 71 - "CooldownAdvance.cs"
Cohesion: 0.40
Nodes (4): CooldownAdvance, CooldownAdvanceType, float, GameObject

### Community 72 - "EntitySplitting"
Cohesion: 0.15
Nodes (8): EnemySpawner, GameObject, Vector2, EntitySplitting, float, GameObject, int, RuntimeInitializeOnLoadMethod

### Community 73 - "Benchmark01"
Cohesion: 0.17
Nodes (10): Font, IEnumerator, int, Material, string, TextContainer, TextMesh, TextMeshPro (+2 more)

### Community 74 - "Benchmark01_UGUI"
Cohesion: 0.17
Nodes (10): Canvas, Font, IEnumerator, int, Material, string, TextMeshProUGUI, TMP_FontAsset (+2 more)

### Community 75 - "TMP_TextSelector_A"
Cohesion: 0.20
Nodes (6): bool, Camera, int, PointerEventData, TextMeshPro, TMP_TextSelector_A

### Community 76 - "[v0.3.3_2] - 2026-08-30"
Cohesion: 0.40
Nodes (5): Added, Changed, Fixed, Removed, [v0.3.3_2] - 2026-08-30

### Community 77 - "com.unity.ugui"
Cohesion: 0.17
Nodes (12): dependencies, depth, source, version, dependencies, depth, source, url (+4 more)

### Community 78 - "com.unity.2d.common"
Cohesion: 0.05
Nodes (42): com.unity.2d.common, com.unity.collections, dependencies, depth, source, url, version, dependencies (+34 more)

### Community 79 - "ObjectSpin"
Cohesion: 0.18
Nodes (8): float, int, Transform, Vector3, MotionType, ObjectSpin, Color32, MotionType

### Community 80 - "TMP_TextEventCheck"
Cohesion: 0.18
Nodes (3): TMP_Text, TMP_TextEventCheck, TMP_TextEventHandler

### Community 81 - "com.unity.modules.imageconversion"
Cohesion: 0.10
Nodes (21): dependencies, depth, source, version, dependencies, depth, source, version (+13 more)

### Community 82 - "⚠️ [v0.2.13] - 2026-08-27"
Cohesion: 0.50
Nodes (4): Added, Changed, Removed, ⚠️ [v0.2.13] - 2026-08-27

### Community 83 - "StatusEffectManager"
Cohesion: 0.05
Nodes (26): StatusEffect, bool, float, GameObject, int, Sprite, string, Vector2 (+18 more)

### Community 85 - "AttackReplacement"
Cohesion: 0.29
Nodes (3): AttackReplacement, AttackAsset, bool

### Community 86 - "ShaderPropAnimator"
Cohesion: 0.22
Nodes (6): AnimationCurve, float, IEnumerator, Material, ShaderPropAnimator, Renderer

### Community 87 - "[v0.3.3] - 2026-08-30 — Projectile Movement Patterns & Screen-Wide Spawn Lines"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, [v0.3.3] - 2026-08-30 — Projectile Movement Patterns & Screen-Wide Spawn Lines

### Community 88 - "com.unity.modules.ui"
Cohesion: 0.11
Nodes (18): com.unity.modules.hierarchycore, dependencies, depth, source, version, dependencies, depth, source (+10 more)

### Community 90 - "[v0.1.11] - 2026-08-24 - Currency Update"
Cohesion: 0.67
Nodes (3): Added, Updated, [v0.1.11] - 2026-08-24 - Currency Update

### Community 91 - ".ResumeGameLoop"
Cohesion: 0.17
Nodes (4): AnomalyInstance, AttackReward, GeneratedReward, PlayerUpgradeReward

### Community 92 - "[v0.1.10_1] - 2026-08-23"
Cohesion: 0.67
Nodes (3): Added, Updated, [v0.1.10_1] - 2026-08-23

### Community 93 - ".TriggerUpgrades"
Cohesion: 0.36
Nodes (4): GameObject, IEnumerator, Vector2, Vector3

### Community 94 - "ProjectileData"
Cohesion: 0.27
Nodes (10): ApplyCondition, EffectData, MovementType, ProjectileData, SpecialScalingAttribute, bool, EffectAsset, float (+2 more)

### Community 95 - "ProjectileDamageSnapshot"
Cohesion: 0.32
Nodes (6): ProjectileDamageSnapshot, ProjectileSnapshot, bool, float, GameObject, int

### Community 96 - "Updated"
Cohesion: 0.67
Nodes (3): Note, Updated, [v0.1.7_2] - 2026-08-18

### Community 102 - "SimpleScript"
Cohesion: 0.29
Nodes (4): float, string, TextMeshPro, SimpleScript

### Community 104 - "Workflow"
Cohesion: 0.15
Nodes (12): 1. Get Git Diff, 2. Analyze Changes, 3. Update CHANGELOG.md, 4. Update README.md, 5. Update ROADMAP.md, 6. Update TODO.md, 7. Update GAME.md, Example Prompt (+4 more)

### Community 105 - "TeleType"
Cohesion: 0.29
Nodes (4): IEnumerator, string, TMP_Text, TeleType

### Community 106 - "[v0.3.1] - 2026-08-29 — Compiler-Enforced Assembly Boundaries"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, [v0.3.1] - 2026-08-29 — Compiler-Enforced Assembly Boundaries

### Community 107 - ".OnMilestoneRewardClaimed"
Cohesion: 0.33
Nodes (3): List, MilestoneReward, MilestoneRewardData

### Community 108 - "TextIndicatorSpawner"
Cohesion: 0.24
Nodes (6): TextIndicatorSpawner, Canvas, int, List, Queue, TextIndicator

### Community 109 - "[v0.0.22] - 2026-08-06"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.0.22] - 2026-08-06

### Community 111 - "[v0.1.5] - 2026-08-10"
Cohesion: 0.50
Nodes (4): Added, Fixed, Rebalance, [v0.1.5] - 2026-08-10

### Community 112 - "⚠️ [v0.2.9] - 2026-08-26 - Stat System Refactor"
Cohesion: 0.50
Nodes (4): Changed, Fixed, Removed, ⚠️ [v0.2.9] - 2026-08-26 - Stat System Refactor

### Community 113 - "[v0.1.10_2] - 2026-08-23"
Cohesion: 0.40
Nodes (5): Added, Fixed, Rebalance, Update, [v0.1.10_2] - 2026-08-23

### Community 114 - "StellarSurge"
Cohesion: 0.50
Nodes (3): float, GameObject, StellarSurge

### Community 115 - "TMP_ExampleScript_01"
Cohesion: 0.22
Nodes (7): bool, int, string, TMP_Text, objectType, TMP_ExampleScript_01, objectType

### Community 116 - "PlayerUpgradeManager"
Cohesion: 0.15
Nodes (7): PlayerUpgradeManager, bool, Dictionary, HashSet, List, IOnHitEffect, IUpgradeHolder

### Community 118 - "[v0.2.7] - 2026-08-26 - Enemy Scaling Overhaul"
Cohesion: 0.50
Nodes (4): Changed, Fixed, Updated, [v0.2.7] - 2026-08-26 - Enemy Scaling Overhaul

### Community 119 - "WarpTextExample"
Cohesion: 0.27
Nodes (5): AnimationCurve, float, IEnumerator, TMP_Text, WarpTextExample

### Community 120 - "[v0.2.4] - 2026-08-25 - Gamemode Selector Update"
Cohesion: 0.40
Nodes (5): Added, Fixed, Removed, Update, [v0.2.4] - 2026-08-25 - Gamemode Selector Update

### Community 121 - "[v0.2.6] - 2026-08-26 - Skill Tree Enhancement Update"
Cohesion: 0.67
Nodes (3): Added, Rebalance, [v0.2.6] - 2026-08-26 - Skill Tree Enhancement Update

### Community 122 - "[v0.1.12] - 2026-08-24 - Milestone Rewards Update"
Cohesion: 0.50
Nodes (4): Added, Fixed, Rebalance, [v0.1.12] - 2026-08-24 - Milestone Rewards Update

### Community 123 - "DropdownSample"
Cohesion: 0.40
Nodes (3): TextMeshProUGUI, DropdownSample, TMP_Dropdown

### Community 124 - "[v0.0.13] - 2026-07-31"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, [v0.0.13] - 2026-07-31

### Community 125 - "TooltipUI"
Cohesion: 0.29
Nodes (4): TooltipUI, RectTransform, TextMeshProUGUI, Vector2

### Community 126 - "[v0.0.9_1] - 2026-07-29"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.0.9_1] - 2026-07-29

### Community 127 - "[v0.1.6] - 2026-08-13 - The Titles Update"
Cohesion: 0.40
Nodes (5): Added, Exodus, Rebalance, Update, [v0.1.6] - 2026-08-13 - The Titles Update

### Community 128 - "[v0.1.2] - 2026-08-10"
Cohesion: 0.50
Nodes (4): Added, Fixed, Removed, [v0.1.2] - 2026-08-10

### Community 129 - "[v0.1.9_1] - 2026-08-23"
Cohesion: 0.40
Nodes (5): Added, Fixed, Removed, Updated, [v0.1.9_1] - 2026-08-23

### Community 131 - "SkillTreeInputToggle"
Cohesion: 0.22
Nodes (3): SkillTreeInputToggle, CallbackContext, PlayerControls

### Community 132 - "[v0.2.2] - 2026-08-25 - Skill Tree Expansion Update"
Cohesion: 0.40
Nodes (5): Added, Changed, Fixed, Rebalance, [v0.2.2] - 2026-08-25 - Skill Tree Expansion Update

### Community 133 - "EntityGearManager"
Cohesion: 0.20
Nodes (8): EntityGearManager, Dictionary, List, StatBuff, StatType, EquipmentSlot, GearItem, StatRoll

### Community 134 - "UnityEvent"
Cohesion: 0.33
Nodes (6): CharacterSelectionEvent, LineSelectionEvent, LinkSelectionEvent, SpriteSelectionEvent, WordSelectionEvent, UnityEvent

### Community 135 - "EnemyPhase"
Cohesion: 0.32
Nodes (5): EnemyPhase, PhaseBuff, float, int, StatBuff

### Community 136 - "[v0.3.5] - 2026-08-30 - Content Expansion Update"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, [v0.3.5] - 2026-08-30 - Content Expansion Update

### Community 137 - "[v0.2.5] - 2026-08-25"
Cohesion: 0.40
Nodes (5): Added, Changed, Removed, Updated, [v0.2.5] - 2026-08-25

### Community 138 - "[v0.0.23] - 2026-08-06"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.0.23] - 2026-08-06

### Community 139 - "MonoBehaviour"
Cohesion: 0.12
Nodes (11): Enemy, Entity, Font, int, Benchmark03, BenchmarkType, int, Transform (+3 more)

### Community 140 - "PlayerMovement"
Cohesion: 0.13
Nodes (11): PlayerMovement, Animator, float, GameObject, IEnumerator, int, IStatProvider, List (+3 more)

### Community 141 - "⚠️ [v0.2.16] - 2026-08-27"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, ⚠️ [v0.2.16] - 2026-08-27

### Community 142 - "⚠️ [v0.2.12] - 2026-08-26"
Cohesion: 0.40
Nodes (5): Added, Changed, Fixed, Removed, ⚠️ [v0.2.12] - 2026-08-26

### Community 143 - "[v0.3.4] - 2026-08-30"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, [v0.3.4] - 2026-08-30

### Community 144 - "[v0.1.3] - 2026-08-10"
Cohesion: 0.67
Nodes (3): Added, Fixed, [v0.1.3] - 2026-08-10

### Community 145 - "com.unity.modules.audio"
Cohesion: 0.09
Nodes (24): dependencies, depth, source, version, dependencies, depth, source, version (+16 more)

### Community 146 - ".BuildDamagePacket"
Cohesion: 0.18
Nodes (9): DamagePacketBuilder, Color, DamagePacket, DamageType, GameObject, ProjectileDamageSnapshot, ProjectileData, damage (+1 more)

### Community 147 - "Decoy"
Cohesion: 0.31
Nodes (7): Color, float, GameObject, IEnumerator, StatusEffect, Vector3, Decoy

### Community 148 - "UnlockEffect"
Cohesion: 0.33
Nodes (4): UnlockEffect, GameObject, List, IUnlockEffect

### Community 149 - "⚠️ [v0.2.8_2] - 2026-08-26"
Cohesion: 0.50
Nodes (4): Changed, Removed, Updated, ⚠️ [v0.2.8_2] - 2026-08-26

### Community 150 - "WaveSequence"
Cohesion: 0.28
Nodes (8): WaveData, WaveSequence, float, GameObject, int, List, string, Vector2

### Community 154 - "⚠️ [v0.2.14] - 2026-08-27"
Cohesion: 0.50
Nodes (4): Added, Changed, Removed, ⚠️ [v0.2.14] - 2026-08-27

### Community 156 - "[v0.1.12_1] - 2026-08-24"
Cohesion: 0.67
Nodes (3): Added, Rebalance, [v0.1.12_1] - 2026-08-24

### Community 158 - "[v0.3.9] - 2026-08-31"
Cohesion: 0.40
Nodes (5): Added, Changed, Fixed, Rebalance, [v0.3.9] - 2026-08-31

### Community 159 - "[v0.3.8_1] - 2026-08-31"
Cohesion: 0.50
Nodes (4): Added, Fixed, Rebalance, [v0.3.8_1] - 2026-08-31

### Community 164 - "[v0.2.1] - 2026-08-25"
Cohesion: 0.67
Nodes (3): Added, Fixed, [v0.2.1] - 2026-08-25

### Community 165 - "com.unity.collab-proxy"
Cohesion: 0.33
Nodes (6): dependencies, depth, source, url, version, com.unity.collab-proxy

### Community 170 - "[v0.3.6] - 2026-08-30"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, [v0.3.6] - 2026-08-30

### Community 172 - "[v0.1.0-PR2] - 2026-08-09"
Cohesion: 0.67
Nodes (3): Added, Fixed, [v0.1.0-PR2] - 2026-08-09

### Community 173 - "copilot-instructions.md"
Cohesion: 0.50
Nodes (3): Code & Architecture, Output Format, Workflow

### Community 174 - "StatReduction"
Cohesion: 0.33
Nodes (4): StatReduction, float, StatBuff, StatType

### Community 175 - "Pulled"
Cohesion: 0.25
Nodes (4): Pulled, bool, float, Vector2

### Community 177 - "[v0.3.8] - 2026-08-31"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, [v0.3.8] - 2026-08-31

### Community 187 - "[v0.0.21] - 2026-08-05 — Knockback Update Part 2"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.0.21] - 2026-08-05 — Knockback Update Part 2

### Community 188 - "GainMana"
Cohesion: 0.50
Nodes (3): GameObject, int, GainMana

### Community 189 - "⚠️ [v0.2.11] - 2026-08-26"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, ⚠️ [v0.2.11] - 2026-08-26

### Community 190 - "[v0.0.19] - 2026-08-05"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.0.19] - 2026-08-05

### Community 191 - "[v0.3.3_1] - 2026-08-30"
Cohesion: 0.33
Nodes (6): Added, Changed, Credits, Fixed, Removed, [v0.3.3_1] - 2026-08-30

### Community 207 - "TextIndicator"
Cohesion: 0.22
Nodes (7): TextIndicator, Camera, Color, float, TextMeshProUGUI, Vector2, Vector3

### Community 208 - "EntityStats"
Cohesion: 0.22
Nodes (6): EnemyStats, EntityStats, bool, float, int, PlayerStats

### Community 209 - ".AddStat"
Cohesion: 0.32
Nodes (3): StatBuff, StatType, StatType

### Community 210 - "[v0.1.8_5] - 2026-08-22"
Cohesion: 0.67
Nodes (3): Changed, Fixed, [v0.1.8_5] - 2026-08-22

### Community 213 - ".ChangeHealth"
Cohesion: 0.21
Nodes (5): Coroutine, GameObject, Color, DamageInstance, DamagePacket

### Community 215 - "[v0.2.0] - 2026-08-24 — Progression, Economy & Milestones Update (Release Summary)"
Cohesion: 0.67
Nodes (3): Highlights, Rebalance, [v0.2.0] - 2026-08-24 — Progression, Economy & Milestones Update (Release Summary)

### Community 216 - "com.unity.modules.unitywebrequest"
Cohesion: 0.06
Nodes (34): dependencies, depth, source, version, dependencies, depth, source, version (+26 more)

### Community 217 - ".SpawnTextIndicator"
Cohesion: 0.60
Nodes (3): Color, IEnumerator, Vector2

### Community 221 - "[v0.1.13] - 2026-08-24 - Stats Extension Update"
Cohesion: 0.67
Nodes (3): Added, Updated, [v0.1.13] - 2026-08-24 - Stats Extension Update

### Community 223 - "CrystalFlux.SkillTree"
Cohesion: 0.15
Nodes (6): Player, NodeRequirement, GameObject, List, CrystalFlux.SkillTree, IUnlockRequirement

### Community 225 - "[v0.1.11_1] - 2026-08-24"
Cohesion: 0.50
Nodes (4): Fixed, Rebalance, Updated, [v0.1.11_1] - 2026-08-24

### Community 231 - "VertexColorCycler"
Cohesion: 0.33
Nodes (3): IEnumerator, TMP_Text, VertexColorCycler

### Community 243 - "PlayerAttackCooldownUI"
Cohesion: 0.17
Nodes (12): PlayerAttackCooldownUI, AttackType, Image, IStatProvider, ITooltipDisplay, offset, PointerEventData, string (+4 more)

### Community 246 - "PlayerDashCooldownUI"
Cohesion: 0.19
Nodes (7): PlayerDashCooldownUI, Image, offset, PointerEventData, subtitle, title, Vector2

### Community 253 - "Reminiscence"
Cohesion: 0.40
Nodes (4): bool, GameObject, StatusEffect, Reminiscence

### Community 254 - "[v0.0.1] - 2026-07-29 — Initial Release"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.0.1] - 2026-07-29 — Initial Release

### Community 255 - "[v0.2.8] - 2026-08-26"
Cohesion: 0.67
Nodes (3): Removed, Updated, [v0.2.8] - 2026-08-26

### Community 256 - "Added"
Cohesion: 0.50
Nodes (4): Added, Fixed, Updated, [v0.1.9] - 2026-08-23 - Level Update

### Community 258 - "SpawnProjectile"
Cohesion: 0.67
Nodes (3): GameObject, Vector2, SpawnProjectile

### Community 260 - "[v0.3.2] - 2026-08-29 — Core Extracted to a Package, Wave Decoupled"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, [v0.3.2] - 2026-08-29 — Core Extracted to a Package, Wave Decoupled

### Community 261 - "⚠️ [v0.2.18] - 2026-08-28"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, ⚠️ [v0.2.18] - 2026-08-28

### Community 266 - "[v0.3.7] - 2026-08-31"
Cohesion: 0.67
Nodes (3): Changed, Fixed, [v0.3.7] - 2026-08-31

### Community 268 - "[v0.2.19] - 2026-08-29"
Cohesion: 0.50
Nodes (4): Added, Changed, Updated, [v0.2.19] - 2026-08-29

### Community 282 - "AttackData"
Cohesion: 0.05
Nodes (39): PlayerAttackHandler, Animator, AttackAsset, AttackType, Dictionary, GameObject, hp, IDamageable (+31 more)

### Community 303 - "GrantStatusEffect"
Cohesion: 0.27
Nodes (6): GameObject, int, List, Vector2, GrantStatusEffect, PlayerUpgrade

### Community 312 - "[v0.0.2] - 2026-07-29"
Cohesion: 0.67
Nodes (3): Added, Fixed, [v0.0.2] - 2026-07-29

### Community 314 - "DamageCalculator"
Cohesion: 0.22
Nodes (8): DamageCalculator, AttackType, DamageType, IStatProvider, ProjectileDamageSnapshot, ProjectileData, dmg, size

### Community 315 - "[v0.1.10_3] - 2026-08-24"
Cohesion: 0.67
Nodes (3): Added, Updated, [v0.1.10_3] - 2026-08-24

### Community 318 - "GameController"
Cohesion: 0.14
Nodes (9): Color, Coroutine, GameObject, IEnumerator, TextMeshProUGUI, TMP_Text, GameController, IAnnouncer (+1 more)

### Community 360 - "com.unity.modules.jsonserialize"
Cohesion: 0.08
Nodes (28): com.unity.modules.subsystems, dependencies, depth, source, version, dependencies, depth, source (+20 more)

## Knowledge Gaps
- **749 isolated node(s):** `CrystalFlux.Utils`, `Added`, `Changed`, `Fixed`, `Rebalance` (+744 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **167 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `WaveManager` connect `WaveManager` to `CrystalFlux.WaveSystem`, `.WaveSpawnRoutine`, `UnlimitedWaveManager`, `.OnMilestoneRewardClaimed`, `MonoBehaviour`, `EntityStatManager`, `PlayerUpgradeManager`, `PlayerSkillTree`, `.GetCurrentWave`, `AttackData`, `.ResumeGameLoop`, `GameController`, `AnomalyInstance`?**
  _High betweenness centrality (0.057) - this node is a cross-community bridge._
- **Why does `CrystalFlux.Core` connect `CrystalFlux.Core` to `StatBuffs`, `EnemyStatManager`, `CooldownAdvance.cs`, `RewardButton`, `TextIndicatorSpawner`, `Pulled`, `EntityStats`, `EntityStatManager`, `UnlockEffect`, `AttackReplacement`, `Item`, `CrystalFlux.StatusEffectSystem`, `AttackData`, `AnomalyInstance`, `GameController`, `CrystalFlux.SkillTree`?**
  _High betweenness centrality (0.045) - this node is a cross-community bridge._
- **Why does `UnityEngine.UI` connect `CrystalFlux.Core` to `Anamnesis.slnx`, `CrystalFlux.SkillTree`, `CrystalFlux.WaveSystem`, `RewardButton`, `Benchmark01_UGUI`, `TMPro`, `StatusEffectManager`, `PlayerDashCooldownUI`, `ChatController`, `AnomalyInstance`?**
  _High betweenness centrality (0.036) - this node is a cross-community bridge._
- **What connects `CrystalFlux.Utils`, `Added`, `Changed` to the rest of the system?**
  _749 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Anamnesis.slnx` be split into smaller, more focused modules?**
  _Cohesion score 0.020202020202020204 - nodes in this community are weakly interconnected._
- **Should `Projectile` be split into smaller, more focused modules?**
  _Cohesion score 0.11384615384615385 - nodes in this community are weakly interconnected._
- **Should `@PlayerControls` be split into smaller, more focused modules?**
  _Cohesion score 0.06262626262626263 - nodes in this community are weakly interconnected._