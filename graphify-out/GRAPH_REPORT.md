# Graph Report - Anamnesis  (2026-09-25)

## Corpus Check
- 198 files · ~708,515 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3922 nodes · 5826 edges · 508 communities (276 shown, 232 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 167 edges (avg confidence: 0.81)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `2d2728c8`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- Anamnesis.slnx
- Projectile
- @PlayerControls
- com.unity.mathematics
- WaveReward.cs
- com.unity.render-pipelines.core
- EntityProjectileHandler
- StatusEffectManager
- CollectibleSpawner
- Planned Features
- EnemyAttackHandler
- Changelog
- com.unity.test-framework
- com.unity.modules.imgui
- PrefabPool
- EntityStatManager
- Rare Pool
- RushState
- TMP_TextEventHandler
- dependencies
- bool
- Player Upgrades
- PlayerSkillTree
- TMP_TextInfoDebugTool
- TMP_TextSelector_B
- AnomalyInstance
- PlayerUpgrade
- com.unity.modules.uielements
- CombatFeedback
- CollectibleData
- TypeSelectorDrawer
- HoverScale
- SkillNodeUI
- EntityHealth
- GameController
- DeathScreenUI
- PlayerAttackCooldownUI
- .GenerateMixedPool
- EnemyMovement
- com.unity.2d.tilemap
- Collectible
- UnlimitedWaveManager
- TextMeshProFloatingText
- EntitySummonHandler
- SkillTreeUI
- SkewTextExample
- TMPro
- RewardButton
- MirageClone
- GAME.md
- SkillTreeLineRenderer
- VertexJitter
- dependencies
- BossBarUI
- PlayerInputHandler
- TooltipTrigger
- CameraController
- MirageStatManager
- DifficultySelector
- com.unity.modules.physics
- EtherealMirage
- PlayerMovement
- SoulRend
- VertexShakeA
- TextConsoleSimulator
- VertexZoom
- AnomalySplitter
- VertexShakeB
- com.crystalflux.core
- com.unity.ext.nunit
- GameInput
- PlayerResourcePool
- PlayerUpgradeManager
- Benchmark01
- Benchmark01_UGUI
- TMP_TextSelector_A
- [v0.3.3_2] - 2026-08-30
- TextIndicator
- com.unity.2d.common
- ObjectSpin
- TMP_TextEventCheck
- com.unity.modules.imageconversion
- [v0.3.1] - 2026-08-29 — Compiler-Enforced Assembly Boundaries
- PlayerUI
- AttackAsset
- SkillTreePanZoom
- ShaderPropAnimator
- [v0.3.3] - 2026-08-30 — Projectile Movement Patterns & Screen-Wide Spawn Lines
- com.unity.modules.ui
- .HandleHitEntity
- [v0.1.12] - 2026-08-24 - Milestone Rewards Update
- SkillTreeRefundAllButton
- [v0.1.10_2] - 2026-08-23
- EntitySplitting
- ProjectileData
- Targetable
- AddChain
- com.unity.modules.physics2d
- com.unity.modules.unitywebrequestaudio
- EnemyStatManager
- bool
- .GetCurrentWave
- AttackType
- EnemyPhase
- Workflow
- UnlockEffect
- [v0.3.2] - 2026-08-29 — Core Extracted to a Package, Wave Decoupled
- AnomalyButtonUI
- .Acquire
- RebindButtonUI
- MenuPause
- [v0.1.5] - 2026-08-10
- ⚠️ [v0.2.18] - 2026-08-28
- DoTSpread
- TMPro.Examples
- com.unity.ugui
- .HandleMovement
- Color
- ⚠️ [v0.2.12] - 2026-08-26
- WarpTextExample
- [v0.2.4] - 2026-08-25 - Gamemode Selector Update
- EnvMapAnimator
- SkillTreeInputToggle
- RestartConfirmPanelUI
- [v0.0.21] - 2026-08-05 — Knockback Update Part 2
- Roadmap
- ProjectileSpawner
- [v0.1.6] - 2026-08-13 - The Titles Update
- [v0.1.2] - 2026-08-10
- GameObject
- Dictionary
- [v0.2.2] - 2026-08-25 - Skill Tree Expansion Update
- EntityGearManager
- GrantStatusEffect
- Canvas
- [v0.3.5] - 2026-08-30 - Content Expansion Update
- [v0.2.5] - 2026-08-25
- TMP_UiFrameRateCounter
- StatBuff
- [v0.4.10] - 2026-09-08
- .BuildDamagePacket
- [v0.1.3] - 2026-08-10
- [v0.3.4] - 2026-08-30
- TMP_FrameRateCounter
- com.unity.modules.audio
- StatReduction
- [v0.6.0] - 2026-09-21 - Impact & Efficiency (Release Summary)
- [v0.1.9_1] - 2026-08-23
- ⚠️ [v0.2.11] - 2026-08-26
- ⚠️ [v0.2.8_2] - 2026-08-26
- com.crystalflux.core
- com.unity.2d.animation
- com.unity.2d.aseprite
- Anamnesis
- com.unity.2d.psdimporter
- Overhealth
- com.unity.2d.spriteshape
- [v0.3.11] - 2026-08-31
- [v0.3.9] - 2026-08-31
- com.unity.2d.tilemap.extras
- com.unity.2d.tooling
- com.unity.cinemachine
- com.unity.collab-proxy
- .CalculateStatGains
- ChatController
- com.unity.ide.rider
- Agent skills
- com.unity.inputsystem
- com.unity.modules.androidjni
- [v0.3.6] - 2026-08-30
- com.unity.modules.cloth
- AttackData
- copilot-instructions.md
- .TakeDamage
- [v0.4.0] - 2026-09-01 — Object Pooling Update (Release Summary)
- .TriggerUpgrades
- [v0.3.8_1] - 2026-08-31
- DoT
- com.unity.modules.unitywebrequesttexture
- com.unity.modules.unitywebrequestwww
- com.unity.modules.vehicles
- com.unity.modules.video
- com.unity.modules.wind
- com.unity.multiplayer.center
- com.unity.render-pipelines.universal
- com.unity.2d.sprite
- ProjectileDamageSnapshot
- Issue tracker: GitHub
- [v0.4.1_2] - 2026-09-02
- AttackType
- [v0.3.3_1] - 2026-08-30
- com.unity.2d.tilemap
- com.unity.modules.vectorgraphics
- com.unity.modules.animation
- com.unity.modules.assetbundle
- com.unity.modules.imgui
- com.unity.modules.jsonserialize
- com.unity.modules.particlesystem
- com.unity.modules.physics
- com.unity.modules.tilemap
- com.unity.modules.ui
- com.unity.modules.uielements
- com.unity.modules.xr
- com.unity.test-framework
- com.unity.ugui
- com.unity.modules.audio
- [v0.4.4] - 2026-09-04 — Ironman Update
- pull_request_template.md
- .RunAttack
- [v0.1.10_1] - 2026-08-23
- [v0.3.8] - 2026-08-31
- com.unity.modules.terrain
- [v0.5.3] - 2026-09-14
- .ApplyKnockback
- Pulled
- com.unity.modules.unitywebrequest
- float
- [v0.5.0] - 2026-09-10 - Feel & Foundations (Release Summary)
- DifficultyData
- [v0.1.11_1] - 2026-08-24
- [v0.3.10] - 2026-08-31
- StatType
- GameObject
- Color
- [v0.4.0_1] - 2026-09-01
- float
- [v0.4.1] - 2026-09-01 — Settings Menu
- TooltipUI
- GameObject
- [v0.6.2] - 2026-09-22
- GameSettings
- bool
- Color
- float
- Entity
- GameObject
- List
- Vector2
- GameObject
- Vector3
- StatBuff
- GameObject
- EntityStats
- List
- Vector2
- IPointerEnterHandler
- Vector2
- [v0.2.7] - 2026-08-26 - Enemy Scaling Overhaul
- Vector2
- GameObject
- GameObject
- [v0.2.1] - 2026-08-25
- [v0.4.9] - 2026-09-06
- TMP_ExampleScript_01
- PlayerLevel
- .InitializeHealthBar
- [v0.3.12] - 2026-08-31 - Cast Time Update
- Domain Docs
- .HandlePatternMovement
- DropdownSample
- [v0.4.5_1] - 2026-09-04
- PropertyAttribute
- SoulRendPU
- TextIndicatorSpawner
- com.unity.collab-proxy
- [v0.4.3] - 2026-09-03 — Difficulty Selector Update
- EnemySpawner
- [v0.1.8_5] - 2026-08-22
- ⚠️ [v0.2.14] - 2026-08-27
- UnityEngine.UI
- AdditionalDamage
- StatusEffect
- IOrbitRegister
- Reminiscence
- ⚠️ [v0.2.9] - 2026-08-26 - Stat System Refactor
- IronmanSelector
- Array
- IEnumerator
- Quaternion
- TMP_FontAsset
- .GetCosts
- Benchmark03
- bool
- float
- bool
- Color
- float
- float
- Vector2
- float
- StatBuff
- List
- float
- StatBuff
- AnomalyData
- Balance
- ⚠️ [v0.2.13] - 2026-08-27
- [v0.4.2] - 2026-09-02
- [v0.1.10_3] - 2026-08-24
- [v0.3.13_1] - 2026-09-01
- Lifesteal
- [v0.4.8] - 2026-09-05
- [v0.2.6] - 2026-08-26 - Skill Tree Enhancement Update
- Skill Tree Attacks
- ControlsPanelUI
- [v0.2.0] - 2026-08-24 — Progression, Economy & Milestones Update (Release Summary)
- [v0.1.11] - 2026-08-24 - Currency Update
- PlayerLevel.cs
- Tooltip
- CooldownAdvance
- DuelInstance
- PointerEventData
- [v0.6.1] - 2026-09-22
- IPoolable
- Action
- IAnnouncer
- CrystalFlux.Core
- MonoBehaviour
- [v0.2.19] - 2026-08-29
- [v0.1.13] - 2026-08-24 - Stats Extension Update
- [v0.4.9_1] - 2026-09-07
- offset
- com.unity.ide.visualstudio
- subtitle
- title
- EffectData
- TMP_Text
- [v0.4.2_2] - 2026-09-03
- AttackReplacement
- [v0.0.19] - 2026-08-05
- [v0.5.1] - 2026-09-13
- VertexColorCycler
- [v0.0.9_1] - 2026-07-29
- Treasure Pool Attacks
- ICurrencyHolder
- .ResumeGameLoop
- AttackType
- .Resolve
- com.unity.modules.director
- StatBuff
- [v0.5.2] - 2026-09-13
- Added
- com.unity.modules.screencapture
- [v0.4.9_2] - 2026-09-08
- [v0.5.5] - 2026-09-17
- [v0.4.5_2] - 2026-09-05
- EffectData
- HashSet
- ISkillPointHolder
- [v0.6.4] - 2026-09-23
- [v0.1.0-PR2] - 2026-08-09
- AttackType
- ProjectileDamageSnapshot
- ITooltipDisplay
- ProjectileData
- [v0.5.7] - 2026-09-21
- [v0.0.5] - 2026-07-29
- com.unity.modules.jsonserialize
- [v0.5.5_1] - 2026-09-18
- GainMana
- [v0.4.6] - 2026-09-05
- Canvas
- ⚠️ [v0.6.3] - 2026-09-23
- Dictionary
- ICurrencyHolder
- [v0.5.0_3] - 2026-09-11
- [v0.0.13] - 2026-07-31
- ISkillPointHolder
- IChargeRegister
- Scene
- com.unity.modules.umbra
- com.unity.modules.unitywebrequest
- StatusEffectCooldownUI
- [v0.3.7] - 2026-08-31
- [v0.0.1] - 2026-07-29 — Initial Release
- [v0.2.8] - 2026-08-26
- com.unity.modules.vr
- [v0.5.6] - 2026-09-18
- [v0.4.11] - 2026-09-10
- [v0.4.8_1] - 2026-09-05
- [v0.4.5] - 2026-09-04 — Golem Update
- com.unity.visualscripting
- Button
- ICurrencyHolder
- ISkillPointHolder
- [v0.1.9_2] - 2026-08-23
- TextIndicator
- [v0.0.9] - 2026-07-29
- [v0.5.0_2] - 2026-09-11
- CallbackContext
- ⚠️ [v0.2.17] - 2026-08-28
- [v0.0.23_2] - 2026-08-06
- ⚠️ [v0.2.16] - 2026-08-27
- .GetExtraEffects
- [v0.0.18] - 2026-08-05 — Rare Stats Update
- InputAction
- PlayerAttackHandler
- GameSettingsLifecycle
- [v0.0.15] - 2026-08-04
- [v0.1.12_1] - 2026-08-24
- ProjectileData
- [v0.4.3_2] - 2026-09-03
- [v0.1.6_1] - 2026-08-15
- [v0.0.23] - 2026-08-06
- Updated
- com.unity.modules.terrainphysics
- com.unity.timeline
- [v0.0.10] - 2026-07-30
- int
- [v0.0.9_2] - 2026-07-29
- AttackReward
- Coroutine
- GeneratedReward
- MilestoneRewardData
- [v0.0.6] - 2026-07-29
- ⚠️ [v0.2.15_1] - 2026-08-27
- PlayerUpgradeReward
- Animator
- RuntimeInitializeOnLoadMethod
- HashSet
- Image
- AttackType
- offset
- PointerEventData
- StatBuffs
- Collision2D
- subtitle
- title
- IEnumerator
- IStatusEffectReceiver
- ProjectileDamageSnapshot
- StatType
- .Setup
- Sprite
- StatusEffect
- string
- triage-labels.md
- AttackAsset
- AttackAsset
- DamageType
- IAttackHandler
- bool
- PlayerUpgradeManager
- Camera
- Coroutine
- CallbackContext
- PlayerControls
- DifficultyData
- Dictionary
- HashSet
- List
- IEnumerator
- InputActionAsset
- InputBinding
- List
- PlayerControls
- canUnlock
- failMessage
- Rigidbody2D
- StatBuff
- RushState
- RectTransform
- Thorns
- SkillTreeOpenButton
- Transform
- WaitForSeconds
- float
- AttackData
- GameObject
- IEnumerator
- RuntimeInitializeOnLoadMethod
- WaveManager
- hp
- ProjectileData
- string
- TextMeshProUGUI
- Slider
- AttackData
- RushState
- Action
- Sprite
- IStatProvider
- string
- StatType
- Dictionary
- IEnumerator
- IUpgradeHolder
- Dictionary
- SkillNodeDef
- [v0.3.13] - 2026-09-01 - Charged Attacks Update
- CrystalFlux.EntitySystem
- DamagePacket
- Dictionary
- EffectAsset
- HashSet
- IReadOnlyList
- List
- PlayerUpgrade
- UpgradeAsset
- Canvas
- [v0.6.6] - 2026-09-25
- [v0.0.22] - 2026-08-06
- [v0.0.2] - 2026-07-29
- [v0.5.0_4] - 2026-09-11
- WaveManager

## God Nodes (most connected - your core abstractions)
1. `Changelog` - 166 edges
2. `WaveManager` - 112 edges
3. `Projectile` - 84 edges
4. `PlayerAttackHandler` - 63 edges
5. `CrystalFlux.Core` - 62 edges
6. `CrystalFlux.EntitySystem` - 48 edges
7. `EntityHealth` - 47 edges
8. `PlayerSkillTree` - 38 edges
9. `EnemyMovement` - 35 edges
10. `PlayerUpgradeManager` - 34 edges

## Surprising Connections (you probably didn't know these)
- `MirageClone` --references--> `EntityHealth`  [EXTRACTED]
  Assets/scripts/Entity/Mirage/MirageClone.cs → Assets/scripts/Entity/EntityHealth.cs
- `PlayerUI` --references--> `EntityHealth`  [EXTRACTED]
  Assets/scripts/Entity/Player/PlayerUI.cs → Assets/scripts/Entity/EntityHealth.cs
- `MirageStatManager` --inherits--> `EntityStatManager`  [EXTRACTED]
  Assets/scripts/Entity/Mirage/MirageStatManager.cs → Assets/scripts/Entity/EntityStatManager.cs
- `MirageClone` --references--> `MirageStatManager`  [EXTRACTED]
  Assets/scripts/Entity/Mirage/MirageClone.cs → Assets/scripts/Entity/Mirage/MirageStatManager.cs
- `MirageClone` --implements--> `IOwnerProxy`  [EXTRACTED]
  Assets/scripts/Entity/Mirage/MirageClone.cs → Assets/scripts/Projectile/IOwnerProxy.cs

## Import Cycles
- None detected.

## Communities (508 total, 232 thin omitted)

### Community 0 - "Anamnesis.slnx"
Cohesion: 0.02
Nodes (100): Assembly-CSharp, Assembly-CSharp-Editor, CrystalFlux.Collectible, CrystalFlux.Entity, CrystalFlux.Pooling, CrystalFlux.Projectile, CrystalFlux.Settings, CrystalFlux.StatusEffect (+92 more)

### Community 1 - "Projectile"
Cohesion: 0.10
Nodes (13): Projectile, bool, Camera, float, IDamageable, int, IResourcePool, List (+5 more)

### Community 2 - "@PlayerControls"
Cohesion: 0.06
Nodes (20): CallbackContext, InputAction, IPlayerActions, IUIActions, PlayerActions, @PlayerControls, UIActions, IDisposable (+12 more)

### Community 3 - "com.unity.mathematics"
Cohesion: 0.10
Nodes (20): com.unity.burst, com.unity.mathematics, dependencies, depth, source, url, version, dependencies (+12 more)

### Community 4 - "WaveReward.cs"
Cohesion: 0.13
Nodes (21): float, IStatProvider, StatType, string, StatModifierInstance, AttackReward, BaseReward, GeneratedReward (+13 more)

### Community 5 - "com.unity.render-pipelines.core"
Cohesion: 0.07
Nodes (30): com.unity.render-pipelines.core, com.unity.render-pipelines.universal-config, com.unity.searcher, com.unity.shadergraph, dependencies, depth, source, version (+22 more)

### Community 6 - "EntityProjectileHandler"
Cohesion: 0.12
Nodes (10): EntityProjectileHandler, AttackData, Camera, int, List, Transform, Vector2, Vector3 (+2 more)

### Community 7 - "StatusEffectManager"
Cohesion: 0.10
Nodes (13): StatusEffectManager, bool, EffectAsset, GameObject, IEnumerator, int, IStatProvider, List (+5 more)

### Community 8 - "CollectibleSpawner"
Cohesion: 0.19
Nodes (6): CollectibleSpawner, float, GameObject, int, List, Dictionary

### Community 9 - "Planned Features"
Cohesion: 0.06
Nodes (32): 2. Legion — player summoner, 3. Furnace — heat resource, 4. Duality — stance switching, 6. Tether — linked enemies, Attacks, Available Colors, Awakenings (PlayerUpgrade), Brainstorm: New Attacks & Awakenings (+24 more)

### Community 10 - "EnemyAttackHandler"
Cohesion: 0.11
Nodes (13): EnemyAttackHandler, Animator, bool, float, GameObject, int, IStatProvider, List (+5 more)

### Community 11 - "Changelog"
Cohesion: 0.02
Nodes (114): Added, Added, Added, Added, Added, Added, Added, Added (+106 more)

### Community 12 - "com.unity.test-framework"
Cohesion: 0.08
Nodes (26): com.unity.nuget.mono-cecil, com.unity.test-framework.performance, dependencies, dependencies, depth, source, url, version (+18 more)

### Community 13 - "com.unity.modules.imgui"
Cohesion: 0.08
Nodes (26): com.unity.settings-manager, com.unity.splines, dependencies, depth, source, url, version, dependencies (+18 more)

### Community 14 - "PrefabPool"
Cohesion: 0.16
Nodes (10): PrefabPool, bool, GameObject, int, List, RuntimeInitializeOnLoadMethod, Transform, EntityId (+2 more)

### Community 15 - "EntityStatManager"
Cohesion: 0.12
Nodes (9): EntityStatManager, Dictionary, HashSet, int, List, EntityStats, ICurrencyHolder, IStatProvider (+1 more)

### Community 16 - "Rare Pool"
Cohesion: 0.06
Nodes (32): Aphelion, Astral Nova, Blaze, Blaze Hyperspark, Blaze Spark, Blood Pact, Cosmic Blaze, Exodus (+24 more)

### Community 17 - "RushState"
Cohesion: 0.22
Nodes (7): RushState, bool, Collision2D, float, GameObject, IStatProvider, Vector2

### Community 18 - "TMP_TextEventHandler"
Cohesion: 0.12
Nodes (16): Camera, Canvas, int, TMP_Text, CharacterSelectionEvent, LineSelectionEvent, LinkSelectionEvent, SpriteSelectionEvent (+8 more)

### Community 19 - "dependencies"
Cohesion: 0.07
Nodes (26): dependencies, depth, source, version, dependencies, depth, source, version (+18 more)

### Community 21 - "Player Upgrades"
Cohesion: 0.08
Nodes (25): Autopilot, Chaos Theory, Cresendo, Decoy, Decoy Upgraded (Capstone), Ethereal Mirage (Keystone), Exsanguinate, Feedback Loop (+17 more)

### Community 22 - "PlayerSkillTree"
Cohesion: 0.11
Nodes (12): PlayerSkillTree, bool, canRefund, canUnlock, Dictionary, failMessage, SkillNodeDef, canUndo (+4 more)

### Community 23 - "TMP_TextInfoDebugTool"
Cohesion: 0.20
Nodes (9): bool, Color, float, string, TMP_Text, Transform, Vector3, TMP_TextInfoDebugTool (+1 more)

### Community 24 - "TMP_TextSelector_B"
Cohesion: 0.10
Nodes (13): bool, Camera, Canvas, int, Object, PointerEventData, RectTransform, string (+5 more)

### Community 25 - "AnomalyInstance"
Cohesion: 0.13
Nodes (7): AnomalyInstance, bool, IDamageable, NoDamageTrialInstance, float, TimeTrialInstance, AnomalyInstance

### Community 26 - "PlayerUpgrade"
Cohesion: 0.12
Nodes (12): PlayerUpgrade, TriggerCondition, float, GameObject, List, HexCast, GameObject, Paradox (+4 more)

### Community 27 - "com.unity.modules.uielements"
Cohesion: 0.09
Nodes (22): dependencies, depth, source, url, version, dependencies, depth, source (+14 more)

### Community 28 - "CombatFeedback"
Cohesion: 0.09
Nodes (11): CombatFeedback, float, IDamageable, RuntimeInitializeOnLoadMethod, HitFeedback, HitFeedbackRunner, bool, float (+3 more)

### Community 29 - "CollectibleData"
Cohesion: 0.20
Nodes (9): CollectibleData, CollectibleType, Color, float, GameObject, int, Sprite, StatType (+1 more)

### Community 30 - "TypeSelectorDrawer"
Cohesion: 0.21
Nodes (9): TypeSelectorDrawer, List, string, CrystalFlux.EditorTools, GUIContent, PropertyDrawer, Rect, SerializedProperty (+1 more)

### Community 31 - "HoverScale"
Cohesion: 0.15
Nodes (6): HoverScale, bool, float, Transform, CrystalFlux.Utils, Vector3

### Community 32 - "SkillNodeUI"
Cohesion: 0.14
Nodes (9): SkillNodeUI, GameObject, SkillNodeDef, canUnlock, SkillNodeDef, Image, IPointerClickHandler, PointerEventData (+1 more)

### Community 33 - "EntityHealth"
Cohesion: 0.09
Nodes (18): EntityHealth, Animator, bool, Camera, Color, float, ICastHandler, IEnumerator (+10 more)

### Community 34 - "GameController"
Cohesion: 0.12
Nodes (9): Color, Coroutine, GameObject, IEnumerator, TextMeshProUGUI, TMP_Text, GameController, IAnnouncer (+1 more)

### Community 35 - "DeathScreenUI"
Cohesion: 0.19
Nodes (6): CoroutineHost, DeathScreenUI, bool, CoroutineHost, IEnumerator, TextMeshProUGUI

### Community 36 - "PlayerAttackCooldownUI"
Cohesion: 0.10
Nodes (18): PlayerAttackCooldownUI, AttackData, bool, Color, Coroutine, float, IEnumerator, Image (+10 more)

### Community 37 - ".GenerateMixedPool"
Cohesion: 0.15
Nodes (4): StatType, WaveQuality, List, BaseReward

### Community 38 - "EnemyMovement"
Cohesion: 0.09
Nodes (18): EnemyMovement, Animator, AttackData, bool, float, GameObject, int, IReadOnlyList (+10 more)

### Community 39 - "com.unity.2d.tilemap"
Cohesion: 0.09
Nodes (24): dependencies, depth, dependencies, depth, source, url, version, source (+16 more)

### Community 40 - "Collectible"
Cohesion: 0.12
Nodes (11): Collectible, bool, float, int, Sprite, Transform, CircleCollider2D, Collider2D (+3 more)

### Community 41 - "UnlimitedWaveManager"
Cohesion: 0.09
Nodes (12): GameObject, IStatProvider, EnemySpawnInfo, UnlimitedWaveManager, bool, DifficultyData, float, GameObject (+4 more)

### Community 42 - "TextMeshProFloatingText"
Cohesion: 0.10
Nodes (16): bool, Font, GameObject, IEnumerator, int, Quaternion, TextMesh, TextMeshPro (+8 more)

### Community 43 - "EntitySummonHandler"
Cohesion: 0.20
Nodes (10): EntitySummonHandler, bool, float, GameObject, IEnumerator, int, List, Vector2 (+2 more)

### Community 44 - "SkillTreeUI"
Cohesion: 0.12
Nodes (12): SkillTreeUI, bool, float, GameObject, int, IReadOnlyList, RuntimeInitializeOnLoadMethod, RectTransform (+4 more)

### Community 45 - "SkewTextExample"
Cohesion: 0.27
Nodes (5): AnimationCurve, float, IEnumerator, TMP_Text, SkewTextExample

### Community 46 - "TMPro"
Cohesion: 0.20
Nodes (4): TMP_DigitValidator, TMP_PhoneNumberValidator, TMPro, TMP_InputValidator

### Community 47 - "RewardButton"
Cohesion: 0.15
Nodes (16): Action, Vector2, RewardButton, RewardType, AttackReward, bool, Color, GeneratedReward (+8 more)

### Community 48 - "MirageClone"
Cohesion: 0.13
Nodes (12): MirageClone, AttackData, bool, float, GameObject, IReadOnlyList, List, Rigidbody2D (+4 more)

### Community 49 - "GAME.md"
Cohesion: 0.25
Nodes (7): Collectibles, Corruption Special Pool, Cyclone Cleave, Game Data Reference, Lacerate, Starting Attacks, Status Effects

### Community 50 - "SkillTreeLineRenderer"
Cohesion: 0.15
Nodes (12): SkillTreeLineRenderer, Color, Dictionary, float, HashSet, Image, int, IReadOnlyList (+4 more)

### Community 51 - "VertexJitter"
Cohesion: 0.16
Nodes (7): bool, float, IEnumerator, Object, TMP_Text, VertexAnim, VertexJitter

### Community 52 - "dependencies"
Cohesion: 0.14
Nodes (13): com.unity.modules.accessibility, com.unity.modules.adaptiveperformance, com.unity.modules.ai, com.unity.modules.unityanalytics, dependencies, com.unity.modules.accessibility, com.unity.modules.adaptiveperformance, com.unity.modules.ai (+5 more)

### Community 53 - "BossBarUI"
Cohesion: 0.25
Nodes (7): BossBarUI, Image, int, IStatProvider, Slider, TextMeshProUGUI, IBossBar

### Community 54 - "PlayerInputHandler"
Cohesion: 0.20
Nodes (3): PlayerInputHandler, CallbackContext, PlayerMovement

### Community 55 - "TooltipTrigger"
Cohesion: 0.24
Nodes (6): TooltipTrigger, PointerEventData, string, Vector2, IPointerExitHandler, ITooltipDisplay

### Community 56 - "CameraController"
Cohesion: 0.17
Nodes (8): bool, float, string, Transform, Vector3, CameraController, CameraModes, CameraModes

### Community 57 - "MirageStatManager"
Cohesion: 0.13
Nodes (11): StatType, MirageStatManager, StatShare, Dictionary, float, GameObject, HashSet, IStatProvider (+3 more)

### Community 58 - "DifficultySelector"
Cohesion: 0.16
Nodes (9): DifficultySelector, bool, Button, GameObject, Image, int, List, TextMeshProUGUI (+1 more)

### Community 59 - "com.unity.modules.physics"
Cohesion: 0.07
Nodes (27): dependencies, depth, source, version, dependencies, depth, source, version (+19 more)

### Community 60 - "EtherealMirage"
Cohesion: 0.19
Nodes (6): EtherealMirage, bool, float, GameObject, int, List

### Community 61 - "PlayerMovement"
Cohesion: 0.10
Nodes (13): PlayerMovement, Animator, Camera, Collision2D, float, GameObject, IEnumerator, int (+5 more)

### Community 62 - "SoulRend"
Cohesion: 0.17
Nodes (3): SoulRend, float, StatBuff

### Community 63 - "VertexShakeA"
Cohesion: 0.17
Nodes (6): bool, float, IEnumerator, Object, TMP_Text, VertexShakeA

### Community 64 - "TextConsoleSimulator"
Cohesion: 0.21
Nodes (5): bool, IEnumerator, Object, TMP_Text, TextConsoleSimulator

### Community 65 - "VertexZoom"
Cohesion: 0.17
Nodes (6): bool, float, IEnumerator, Object, TMP_Text, VertexZoom

### Community 66 - "AnomalySplitter"
Cohesion: 0.17
Nodes (10): AnomalySplitter, float, GameObject, int, float, GameObject, int, string (+2 more)

### Community 67 - "VertexShakeB"
Cohesion: 0.17
Nodes (6): bool, float, IEnumerator, Object, TMP_Text, VertexShakeB

### Community 68 - "com.crystalflux.core"
Cohesion: 0.33
Nodes (6): dependencies, depth, hash, source, version, com.crystalflux.core

### Community 69 - "com.unity.ext.nunit"
Cohesion: 0.15
Nodes (13): com.unity.ext.nunit, dependencies, depth, source, version, dependencies, depth, source (+5 more)

### Community 70 - "GameInput"
Cohesion: 0.11
Nodes (8): GameInput, InputActionAsset, int, List, PlayerControls, RuntimeInitializeOnLoadMethod, UIInputBindingSync, InputSystemUIInputModule

### Community 71 - "PlayerResourcePool"
Cohesion: 0.24
Nodes (6): PlayerResourcePool, bool, float, IStatProvider, IResourcePool, ResourceType

### Community 72 - "PlayerUpgradeManager"
Cohesion: 0.14
Nodes (9): PlayerUpgradeManager, bool, HashSet, IAttackEffectSource, IOnHitEffect, IReadOnlyList, IUpgradeHolder, PlayerUpgrade (+1 more)

### Community 73 - "Benchmark01"
Cohesion: 0.17
Nodes (10): Font, IEnumerator, int, Material, string, TextContainer, TextMesh, TextMeshPro (+2 more)

### Community 74 - "Benchmark01_UGUI"
Cohesion: 0.17
Nodes (10): Canvas, Font, IEnumerator, int, Material, string, TextMeshProUGUI, TMP_FontAsset (+2 more)

### Community 75 - "TMP_TextSelector_A"
Cohesion: 0.20
Nodes (6): bool, Camera, int, PointerEventData, TextMeshPro, TMP_TextSelector_A

### Community 76 - "[v0.3.3_2] - 2026-08-30"
Cohesion: 0.40
Nodes (5): Added, Changed, Fixed, Removed, [v0.3.3_2] - 2026-08-30

### Community 77 - "TextIndicator"
Cohesion: 0.16
Nodes (10): TextIndicator, TextType, Camera, Color, float, TextMeshProUGUI, Vector2, Vector3 (+2 more)

### Community 78 - "com.unity.2d.common"
Cohesion: 0.05
Nodes (42): com.unity.2d.common, com.unity.collections, dependencies, depth, source, url, version, dependencies (+34 more)

### Community 79 - "ObjectSpin"
Cohesion: 0.18
Nodes (8): float, int, Transform, Vector3, MotionType, ObjectSpin, Color32, MotionType

### Community 80 - "TMP_TextEventCheck"
Cohesion: 0.18
Nodes (3): TMP_Text, TMP_TextEventCheck, TMP_TextEventHandler

### Community 81 - "com.unity.modules.imageconversion"
Cohesion: 0.10
Nodes (21): dependencies, depth, source, version, dependencies, depth, source, version (+13 more)

### Community 82 - "[v0.3.1] - 2026-08-29 — Compiler-Enforced Assembly Boundaries"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, [v0.3.1] - 2026-08-29 — Compiler-Enforced Assembly Boundaries

### Community 83 - "PlayerUI"
Cohesion: 0.10
Nodes (16): NumberFormat, PlayerResourceUI, GameObject, IStatProvider, offset, PointerEventData, subtitle, title (+8 more)

### Community 85 - "SkillTreePanZoom"
Cohesion: 0.19
Nodes (5): SkillTreePanZoom, bool, float, RectTransform, Vector2

### Community 86 - "ShaderPropAnimator"
Cohesion: 0.22
Nodes (6): AnimationCurve, float, IEnumerator, Material, ShaderPropAnimator, Renderer

### Community 87 - "[v0.3.3] - 2026-08-30 — Projectile Movement Patterns & Screen-Wide Spawn Lines"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, [v0.3.3] - 2026-08-30 — Projectile Movement Patterns & Screen-Wide Spawn Lines

### Community 88 - "com.unity.modules.ui"
Cohesion: 0.11
Nodes (18): com.unity.modules.hierarchycore, dependencies, depth, source, version, dependencies, depth, source (+10 more)

### Community 89 - ".HandleHitEntity"
Cohesion: 0.27
Nodes (3): GameObject, IReadOnlyList, EffectData

### Community 90 - "[v0.1.12] - 2026-08-24 - Milestone Rewards Update"
Cohesion: 0.50
Nodes (4): Added, Fixed, Rebalance, [v0.1.12] - 2026-08-24 - Milestone Rewards Update

### Community 91 - "SkillTreeRefundAllButton"
Cohesion: 0.13
Nodes (8): SkillTreeManager, canRefund, failMessage, GameObject, SkillTreeRefundAllButton, Color, SkillNodeDef, Vector2

### Community 92 - "[v0.1.10_2] - 2026-08-23"
Cohesion: 0.40
Nodes (5): Added, Fixed, Rebalance, Update, [v0.1.10_2] - 2026-08-23

### Community 93 - "EntitySplitting"
Cohesion: 0.22
Nodes (6): GameObject, Vector2, EntitySplitting, float, GameObject, int

### Community 94 - "ProjectileData"
Cohesion: 0.24
Nodes (13): ApplyCondition, EffectData, MovementType, ProjectileData, SpecialScalingAttribute, StatScale, bool, EffectAsset (+5 more)

### Community 95 - "Targetable"
Cohesion: 0.15
Nodes (8): Targetable, GameObject, IDamageable, int, ITeamMember, List, RuntimeInitializeOnLoadMethod, Vector2

### Community 96 - "AddChain"
Cohesion: 0.33
Nodes (4): AddChain, float, GameObject, List

### Community 99 - "EnemyStatManager"
Cohesion: 0.38
Nodes (3): EnemyStatManager, string, EntityStatManager

### Community 101 - ".GetCurrentWave"
Cohesion: 0.17
Nodes (3): IEnumerator, IEnumerator, WaveData

### Community 102 - "AttackType"
Cohesion: 0.23
Nodes (3): AttackData, IEnumerator, AttackType

### Community 103 - "EnemyPhase"
Cohesion: 0.28
Nodes (5): EnemyPhase, PhaseBuff, float, int, StatBuff

### Community 104 - "Workflow"
Cohesion: 0.15
Nodes (12): 1. Get Git Diff, 2. Analyze Changes, 3. Update CHANGELOG.md, 4. Update README.md, 5. Update ROADMAP.md, 6. Update TODO.md, 7. Update GAME.md, Example Prompt (+4 more)

### Community 105 - "UnlockEffect"
Cohesion: 0.33
Nodes (4): UnlockEffect, GameObject, List, IUnlockEffect

### Community 106 - "[v0.3.2] - 2026-08-29 — Core Extracted to a Package, Wave Decoupled"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, [v0.3.2] - 2026-08-29 — Core Extracted to a Package, Wave Decoupled

### Community 107 - "AnomalyButtonUI"
Cohesion: 0.29
Nodes (5): AnomalyButtonUI, Action, AnomalyInstance, TextMeshProUGUI, Vector2

### Community 108 - ".Acquire"
Cohesion: 0.27
Nodes (7): CastBar, Camera, RuntimeInitializeOnLoadMethod, Slider, TextMeshProUGUI, Transform, Vector3

### Community 109 - "RebindButtonUI"
Cohesion: 0.24
Nodes (6): RebindButtonUI, Button, InputAction, int, TextMeshProUGUI, RebindingOperation

### Community 110 - "MenuPause"
Cohesion: 0.12
Nodes (9): GameRestart, bool, RuntimeInitializeOnLoadMethod, Scene, MenuPause, float, int, RuntimeInitializeOnLoadMethod (+1 more)

### Community 111 - "[v0.1.5] - 2026-08-10"
Cohesion: 0.50
Nodes (4): Added, Fixed, Rebalance, [v0.1.5] - 2026-08-10

### Community 112 - "⚠️ [v0.2.18] - 2026-08-28"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, ⚠️ [v0.2.18] - 2026-08-28

### Community 113 - "DoTSpread"
Cohesion: 0.16
Nodes (11): bool, Coroutine, Dictionary, float, GameObject, IEnumerator, IReadOnlyList, List (+3 more)

### Community 114 - "TMPro.Examples"
Cohesion: 0.11
Nodes (9): float, string, TextMeshPro, SimpleScript, IEnumerator, string, TMP_Text, TeleType (+1 more)

### Community 115 - "com.unity.ugui"
Cohesion: 0.17
Nodes (12): dependencies, depth, source, version, dependencies, depth, source, url (+4 more)

### Community 118 - "⚠️ [v0.2.12] - 2026-08-26"
Cohesion: 0.40
Nodes (5): Added, Changed, Fixed, Removed, ⚠️ [v0.2.12] - 2026-08-26

### Community 119 - "WarpTextExample"
Cohesion: 0.27
Nodes (5): AnimationCurve, float, IEnumerator, TMP_Text, WarpTextExample

### Community 120 - "[v0.2.4] - 2026-08-25 - Gamemode Selector Update"
Cohesion: 0.40
Nodes (5): Added, Fixed, Removed, Update, [v0.2.4] - 2026-08-25 - Gamemode Selector Update

### Community 121 - "EnvMapAnimator"
Cohesion: 0.25
Nodes (5): IEnumerator, Material, TMP_Text, Vector3, EnvMapAnimator

### Community 122 - "SkillTreeInputToggle"
Cohesion: 0.12
Nodes (7): SkillTreeInputToggle, CallbackContext, SettingsMenuInputToggle, bool, InputAction, PlayerControls, SettingsPanelUI

### Community 123 - "RestartConfirmPanelUI"
Cohesion: 0.13
Nodes (9): RestartConfirmPanelUI, bool, string, TextMeshProUGUI, SettingsPanelUI, bool, Button, ControlsPanelUI (+1 more)

### Community 124 - "[v0.0.21] - 2026-08-05 — Knockback Update Part 2"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.0.21] - 2026-08-05 — Knockback Update Part 2

### Community 125 - "Roadmap"
Cohesion: 0.14
Nodes (9): Credits, Roadmap, [v0.1.0] - Skill Tree Update (Released), [v0.2.0] - Content Expansion Update (Released), [v0.3.0] - System Refactor & QoL Update, [v0.4.0] - Content Updates, [v0.5.0] - Feel and Foundations, [v0.6.0] - Impact and Efficiency (+1 more)

### Community 126 - "ProjectileSpawner"
Cohesion: 0.22
Nodes (15): Color, float, GameObject, IEnumerator, StatusEffect, Vector3, Decoy, ProjectileSpawner (+7 more)

### Community 127 - "[v0.1.6] - 2026-08-13 - The Titles Update"
Cohesion: 0.40
Nodes (5): Added, Exodus, Rebalance, Update, [v0.1.6] - 2026-08-13 - The Titles Update

### Community 128 - "[v0.1.2] - 2026-08-10"
Cohesion: 0.50
Nodes (4): Added, Fixed, Removed, [v0.1.2] - 2026-08-10

### Community 129 - "GameObject"
Cohesion: 0.32
Nodes (4): NodeRequirement, List, GameObject, IUnlockRequirement

### Community 132 - "[v0.2.2] - 2026-08-25 - Skill Tree Expansion Update"
Cohesion: 0.40
Nodes (5): Added, Changed, Fixed, Rebalance, [v0.2.2] - 2026-08-25 - Skill Tree Expansion Update

### Community 133 - "EntityGearManager"
Cohesion: 0.05
Nodes (38): EntityGearManager, Dictionary, List, StatBuff, StatType, EquipmentSlot, GearItem, StatRoll (+30 more)

### Community 134 - "GrantStatusEffect"
Cohesion: 0.27
Nodes (5): GameObject, int, List, Vector2, GrantStatusEffect

### Community 136 - "[v0.3.5] - 2026-08-30 - Content Expansion Update"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, [v0.3.5] - 2026-08-30 - Content Expansion Update

### Community 137 - "[v0.2.5] - 2026-08-25"
Cohesion: 0.40
Nodes (5): Added, Changed, Removed, Updated, [v0.2.5] - 2026-08-25

### Community 138 - "TMP_UiFrameRateCounter"
Cohesion: 0.11
Nodes (15): float, int, RectTransform, string, TextMeshProUGUI, FpsCounterAnchorPositions, TMP_UiFrameRateCounter, Camera (+7 more)

### Community 140 - "[v0.4.10] - 2026-09-08"
Cohesion: 0.40
Nodes (5): Added, Changed, Fixed, Removed, [v0.4.10] - 2026-09-08

### Community 141 - ".BuildDamagePacket"
Cohesion: 0.13
Nodes (12): float, GameObject, StellarSurge, DamagePacketBuilder, Color, DamagePacket, DamageType, GameObject (+4 more)

### Community 142 - "[v0.1.3] - 2026-08-10"
Cohesion: 0.67
Nodes (3): Added, Fixed, [v0.1.3] - 2026-08-10

### Community 143 - "[v0.3.4] - 2026-08-30"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, [v0.3.4] - 2026-08-30

### Community 144 - "TMP_FrameRateCounter"
Cohesion: 0.18
Nodes (8): Camera, float, int, string, TextMeshPro, Transform, FpsCounterAnchorPositions, TMP_FrameRateCounter

### Community 145 - "com.unity.modules.audio"
Cohesion: 0.09
Nodes (24): dependencies, depth, source, version, dependencies, depth, source, version (+16 more)

### Community 146 - "StatReduction"
Cohesion: 0.29
Nodes (4): StatReduction, float, StatBuff, StatType

### Community 147 - "[v0.6.0] - 2026-09-21 - Impact & Efficiency (Release Summary)"
Cohesion: 0.40
Nodes (5): Added, Changed, Highlights, Rebalance, [v0.6.0] - 2026-09-21 - Impact & Efficiency (Release Summary)

### Community 148 - "[v0.1.9_1] - 2026-08-23"
Cohesion: 0.40
Nodes (5): Added, Fixed, Removed, Updated, [v0.1.9_1] - 2026-08-23

### Community 149 - "⚠️ [v0.2.11] - 2026-08-26"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, ⚠️ [v0.2.11] - 2026-08-26

### Community 150 - "⚠️ [v0.2.8_2] - 2026-08-26"
Cohesion: 0.50
Nodes (4): Changed, Removed, Updated, ⚠️ [v0.2.8_2] - 2026-08-26

### Community 154 - "Anamnesis"
Cohesion: 0.15
Nodes (13): Anamnesis, Assembly Boundaries, Awakening trigger conditions, 📚 Content, 🎮 Controls, 🔁 Core Loop, ✨ Features, Installation (+5 more)

### Community 156 - "Overhealth"
Cohesion: 0.25
Nodes (5): bool, float, GameObject, List, Overhealth

### Community 158 - "[v0.3.11] - 2026-08-31"
Cohesion: 0.40
Nodes (5): Added, Changed, Fixed, Rebalance, [v0.3.11] - 2026-08-31

### Community 159 - "[v0.3.9] - 2026-08-31"
Cohesion: 0.40
Nodes (5): Added, Changed, Fixed, Rebalance, [v0.3.9] - 2026-08-31

### Community 164 - ".CalculateStatGains"
Cohesion: 0.33
Nodes (5): AttackData, hp, IStatProvider, mana, stamina

### Community 165 - "ChatController"
Cohesion: 0.29
Nodes (4): ChatController, TMP_Text, Scrollbar, TMP_InputField

### Community 167 - "Agent skills"
Cohesion: 0.40
Nodes (4): Agent skills, Domain docs, Issue tracker, Triage labels

### Community 170 - "[v0.3.6] - 2026-08-30"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, [v0.3.6] - 2026-08-30

### Community 172 - "AttackData"
Cohesion: 0.14
Nodes (12): AttackData, RushDirection, RushType, bool, float, GameObject, int, IStatProvider (+4 more)

### Community 173 - "copilot-instructions.md"
Cohesion: 0.50
Nodes (3): Code & Architecture, Output Format, Workflow

### Community 174 - ".TakeDamage"
Cohesion: 0.21
Nodes (4): GameObject, ICastHandler, DamageInstance, DamagePacket

### Community 175 - "[v0.4.0] - 2026-09-01 — Object Pooling Update (Release Summary)"
Cohesion: 0.33
Nodes (6): Added, Changed, Fixed, Highlights, Updated, [v0.4.0] - 2026-09-01 — Object Pooling Update (Release Summary)

### Community 176 - ".TriggerUpgrades"
Cohesion: 0.31
Nodes (5): Vector2, GameObject, IEnumerator, Vector2, Vector3

### Community 177 - "[v0.3.8_1] - 2026-08-31"
Cohesion: 0.50
Nodes (4): Added, Fixed, Rebalance, [v0.3.8_1] - 2026-08-31

### Community 178 - "DoT"
Cohesion: 0.09
Nodes (14): Detonator, Color, DamageType, float, DoT, bool, Color, DamageType (+6 more)

### Community 187 - "ProjectileDamageSnapshot"
Cohesion: 0.16
Nodes (12): DamageCalculator, AttackType, DamageType, IStatProvider, ProjectileDamageSnapshot, ProjectileSnapshot, bool, float (+4 more)

### Community 188 - "Issue tracker: GitHub"
Cohesion: 0.29
Nodes (6): Conventions, Issue tracker: GitHub, Pull requests as a triage surface, Wayfinding operations, When a skill says "fetch the relevant ticket", When a skill says "publish to the issue tracker"

### Community 189 - "[v0.4.1_2] - 2026-09-02"
Cohesion: 0.40
Nodes (5): Added, Changed, Fixed, Rebalance, [v0.4.1_2] - 2026-09-02

### Community 191 - "[v0.3.3_1] - 2026-08-30"
Cohesion: 0.33
Nodes (6): Added, Changed, Credits, Fixed, Removed, [v0.3.3_1] - 2026-08-30

### Community 207 - "[v0.4.4] - 2026-09-04 — Ironman Update"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, [v0.4.4] - 2026-09-04 — Ironman Update

### Community 208 - "pull_request_template.md"
Cohesion: 0.29
Nodes (6): Changes, Checklist, Notes for reviewers, Scene / asset impact, Summary, Testing

### Community 210 - "[v0.1.10_1] - 2026-08-23"
Cohesion: 0.67
Nodes (3): Added, Updated, [v0.1.10_1] - 2026-08-23

### Community 211 - "[v0.3.8] - 2026-08-31"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, [v0.3.8] - 2026-08-31

### Community 213 - "[v0.5.3] - 2026-09-14"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.5.3] - 2026-09-14

### Community 214 - ".ApplyKnockback"
Cohesion: 0.33
Nodes (6): AppliedForce, AppliedForce, KnockbackHandler, float, List, Vector2

### Community 215 - "Pulled"
Cohesion: 0.22
Nodes (4): Pulled, bool, float, Vector2

### Community 216 - "com.unity.modules.unitywebrequest"
Cohesion: 0.06
Nodes (34): dependencies, depth, source, version, dependencies, depth, source, version (+26 more)

### Community 218 - "[v0.5.0] - 2026-09-10 - Feel & Foundations (Release Summary)"
Cohesion: 0.50
Nodes (4): Added, Changed, Highlights, [v0.5.0] - 2026-09-10 - Feel & Foundations (Release Summary)

### Community 219 - "DifficultyData"
Cohesion: 0.24
Nodes (8): DifficultyData, Color, float, int, RuntimeInitializeOnLoadMethod, string, Sprite, StringBuilder

### Community 220 - "[v0.1.11_1] - 2026-08-24"
Cohesion: 0.50
Nodes (4): Fixed, Rebalance, Updated, [v0.1.11_1] - 2026-08-24

### Community 221 - "[v0.3.10] - 2026-08-31"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.3.10] - 2026-08-31

### Community 225 - "[v0.4.0_1] - 2026-09-01"
Cohesion: 0.67
Nodes (3): Added, Rebalance, [v0.4.0_1] - 2026-09-01

### Community 227 - "[v0.4.1] - 2026-09-01 — Settings Menu"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, [v0.4.1] - 2026-09-01 — Settings Menu

### Community 228 - "TooltipUI"
Cohesion: 0.29
Nodes (4): TooltipUI, RectTransform, TextMeshProUGUI, Vector2

### Community 230 - "[v0.6.2] - 2026-09-22"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.6.2] - 2026-09-22

### Community 231 - "GameSettings"
Cohesion: 0.38
Nodes (4): GameSettings, bool, int, string

### Community 235 - "Entity"
Cohesion: 0.40
Nodes (3): Enemy, Entity, Player

### Community 243 - "EntityStats"
Cohesion: 0.22
Nodes (6): EnemyStats, EntityStats, bool, float, int, PlayerStats

### Community 246 - "IPointerEnterHandler"
Cohesion: 0.18
Nodes (8): PlayerDashCooldownUI, Image, offset, PointerEventData, subtitle, title, Vector2, IPointerEnterHandler

### Community 248 - "[v0.2.7] - 2026-08-26 - Enemy Scaling Overhaul"
Cohesion: 0.50
Nodes (4): Changed, Fixed, Updated, [v0.2.7] - 2026-08-26 - Enemy Scaling Overhaul

### Community 252 - "[v0.2.1] - 2026-08-25"
Cohesion: 0.67
Nodes (3): Added, Fixed, [v0.2.1] - 2026-08-25

### Community 253 - "[v0.4.9] - 2026-09-06"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, [v0.4.9] - 2026-09-06

### Community 254 - "TMP_ExampleScript_01"
Cohesion: 0.22
Nodes (7): bool, int, string, TMP_Text, objectType, TMP_ExampleScript_01, objectType

### Community 255 - "PlayerLevel"
Cohesion: 0.33
Nodes (4): PlayerLevel, int, IStatProvider, PlayerUpgradeManager

### Community 256 - ".InitializeHealthBar"
Cohesion: 0.32
Nodes (3): Canvas, CanvasScaler, Component

### Community 257 - "[v0.3.12] - 2026-08-31 - Cast Time Update"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, [v0.3.12] - 2026-08-31 - Cast Time Update

### Community 258 - "Domain Docs"
Cohesion: 0.33
Nodes (5): Before exploring, read these, Domain Docs, File structure, Flag ADR conflicts, Use the glossary's vocabulary

### Community 260 - "DropdownSample"
Cohesion: 0.40
Nodes (3): TextMeshProUGUI, DropdownSample, TMP_Dropdown

### Community 261 - "[v0.4.5_1] - 2026-09-04"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, [v0.4.5_1] - 2026-09-04

### Community 263 - "SoulRendPU"
Cohesion: 0.53
Nodes (3): GameObject, SoulRendPU, SoulRend

### Community 264 - "TextIndicatorSpawner"
Cohesion: 0.32
Nodes (3): TextIndicatorSpawner, int, List

### Community 266 - "com.unity.collab-proxy"
Cohesion: 0.33
Nodes (6): dependencies, depth, source, url, version, com.unity.collab-proxy

### Community 267 - "[v0.4.3] - 2026-09-03 — Difficulty Selector Update"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, [v0.4.3] - 2026-09-03 — Difficulty Selector Update

### Community 269 - "[v0.1.8_5] - 2026-08-22"
Cohesion: 0.67
Nodes (3): Changed, Fixed, [v0.1.8_5] - 2026-08-22

### Community 270 - "⚠️ [v0.2.14] - 2026-08-27"
Cohesion: 0.50
Nodes (4): Added, Changed, Removed, ⚠️ [v0.2.14] - 2026-08-27

### Community 271 - "UnityEngine.UI"
Cohesion: 0.10
Nodes (5): CrystalFlux.WaveSystem, CrystalFlux.CollectibleSystem, CrystalFlux.SkillTree, CrystalFlux.SettingsSystem, UnityEngine.UI

### Community 272 - "AdditionalDamage"
Cohesion: 0.40
Nodes (4): AdditionalDamage, DamageType, float, GameObject

### Community 273 - "StatusEffect"
Cohesion: 0.25
Nodes (8): StatusEffect, bool, float, GameObject, int, string, Vector2, EffectAsset

### Community 275 - "Reminiscence"
Cohesion: 0.40
Nodes (4): bool, GameObject, StatusEffect, Reminiscence

### Community 276 - "⚠️ [v0.2.9] - 2026-08-26 - Stat System Refactor"
Cohesion: 0.50
Nodes (4): Changed, Fixed, Removed, ⚠️ [v0.2.9] - 2026-08-26 - Stat System Refactor

### Community 277 - "IronmanSelector"
Cohesion: 0.20
Nodes (6): IronmanSelector, bool, Button, GameObject, string, Vector2

### Community 282 - ".GetCosts"
Cohesion: 0.18
Nodes (6): hp, IStatProvider, finalHpCost, finalStaminaCost, mp, sp

### Community 283 - "Benchmark03"
Cohesion: 0.25
Nodes (5): Font, int, Benchmark03, BenchmarkType, BenchmarkType

### Community 296 - "AnomalyData"
Cohesion: 0.33
Nodes (6): AnomalyData, AnomalyType, bool, float, int, string

### Community 297 - "Balance"
Cohesion: 0.29
Nodes (7): Added, Balance, Buffs, Changed, Nerfs, Rebalances, [v0.6.5] - 2026-09-24

### Community 298 - "⚠️ [v0.2.13] - 2026-08-27"
Cohesion: 0.50
Nodes (4): Added, Changed, Removed, ⚠️ [v0.2.13] - 2026-08-27

### Community 299 - "[v0.4.2] - 2026-09-02"
Cohesion: 0.50
Nodes (4): Added, Changed, Rebalance, [v0.4.2] - 2026-09-02

### Community 300 - "[v0.1.10_3] - 2026-08-24"
Cohesion: 0.67
Nodes (3): Added, Updated, [v0.1.10_3] - 2026-08-24

### Community 301 - "[v0.3.13_1] - 2026-09-01"
Cohesion: 0.50
Nodes (4): Changed, Fixed, Rebalance, [v0.3.13_1] - 2026-09-01

### Community 302 - "Lifesteal"
Cohesion: 0.33
Nodes (3): Lifesteal, Color, float

### Community 303 - "[v0.4.8] - 2026-09-05"
Cohesion: 0.50
Nodes (4): Added, Changed, Notes, [v0.4.8] - 2026-09-05

### Community 304 - "[v0.2.6] - 2026-08-26 - Skill Tree Enhancement Update"
Cohesion: 0.67
Nodes (3): Added, Rebalance, [v0.2.6] - 2026-08-26 - Skill Tree Enhancement Update

### Community 305 - "Skill Tree Attacks"
Cohesion: 0.33
Nodes (6): Astral Disjunction (Capstone), Decoy Burst, Hypernova (Capstone), Skill Tree Attacks, Ultrasonic (Capstone), Warp (Capstone)

### Community 306 - "ControlsPanelUI"
Cohesion: 0.19
Nodes (8): ControlsPanelUI, bool, Button, InputBinding, List, string, TextMeshProUGUI, Toggle

### Community 307 - "[v0.2.0] - 2026-08-24 — Progression, Economy & Milestones Update (Release Summary)"
Cohesion: 0.67
Nodes (3): Highlights, Rebalance, [v0.2.0] - 2026-08-24 — Progression, Economy & Milestones Update (Release Summary)

### Community 308 - "[v0.1.11] - 2026-08-24 - Currency Update"
Cohesion: 0.67
Nodes (3): Added, Updated, [v0.1.11] - 2026-08-24 - Currency Update

### Community 311 - "CooldownAdvance"
Cohesion: 0.40
Nodes (4): CooldownAdvance, CooldownAdvanceType, float, GameObject

### Community 312 - "DuelInstance"
Cohesion: 0.18
Nodes (8): AnomalyInstance, DuelInstance, float, IStatProvider, string, SwarmInstance, float, string

### Community 314 - "[v0.6.1] - 2026-09-22"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.6.1] - 2026-09-22

### Community 318 - "CrystalFlux.Core"
Cohesion: 0.13
Nodes (4): ProjectilePattern, CrystalFlux.StatusEffectSystem, CrystalFlux.UISystem, CrystalFlux.Core

### Community 319 - "MonoBehaviour"
Cohesion: 0.14
Nodes (9): UnlimitedWaveButtonController, bool, int, Benchmark02, int, Transform, Benchmark04, MonoBehaviour (+1 more)

### Community 320 - "[v0.2.19] - 2026-08-29"
Cohesion: 0.50
Nodes (4): Added, Changed, Updated, [v0.2.19] - 2026-08-29

### Community 321 - "[v0.1.13] - 2026-08-24 - Stats Extension Update"
Cohesion: 0.67
Nodes (3): Added, Updated, [v0.1.13] - 2026-08-24 - Stats Extension Update

### Community 322 - "[v0.4.9_1] - 2026-09-07"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.4.9_1] - 2026-09-07

### Community 330 - "[v0.4.2_2] - 2026-09-03"
Cohesion: 0.50
Nodes (4): Added, Changed, Rebalance, [v0.4.2_2] - 2026-09-03

### Community 332 - "[v0.0.19] - 2026-08-05"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.0.19] - 2026-08-05

### Community 333 - "[v0.5.1] - 2026-09-13"
Cohesion: 0.40
Nodes (5): Added, Changed, Fixed, Rebalance, [v0.5.1] - 2026-09-13

### Community 334 - "VertexColorCycler"
Cohesion: 0.33
Nodes (3): IEnumerator, TMP_Text, VertexColorCycler

### Community 335 - "[v0.0.9_1] - 2026-07-29"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.0.9_1] - 2026-07-29

### Community 336 - "Treasure Pool Attacks"
Cohesion: 0.29
Nodes (7): Autopilot, Chaos Theory, Feedback Loop, Soul Fragment, Soul Rend, Supersonic, Treasure Pool Attacks

### Community 338 - ".ResumeGameLoop"
Cohesion: 0.13
Nodes (7): AnomalyInstance, List, AttackReward, GeneratedReward, MilestoneReward, MilestoneRewardData, PlayerUpgradeReward

### Community 340 - ".Resolve"
Cohesion: 0.50
Nodes (3): IOwnerProxy, OwnerProxy, GameObject

### Community 343 - "[v0.5.2] - 2026-09-13"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.5.2] - 2026-09-13

### Community 344 - "Added"
Cohesion: 0.50
Nodes (4): Added, Fixed, Updated, [v0.1.9] - 2026-08-23 - Level Update

### Community 346 - "[v0.4.9_2] - 2026-09-08"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.4.9_2] - 2026-09-08

### Community 347 - "[v0.5.5] - 2026-09-17"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.5.5] - 2026-09-17

### Community 348 - "[v0.4.5_2] - 2026-09-05"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.4.5_2] - 2026-09-05

### Community 352 - "[v0.6.4] - 2026-09-23"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.6.4] - 2026-09-23

### Community 353 - "[v0.1.0-PR2] - 2026-08-09"
Cohesion: 0.67
Nodes (3): Added, Fixed, [v0.1.0-PR2] - 2026-08-09

### Community 358 - "[v0.5.7] - 2026-09-21"
Cohesion: 0.67
Nodes (3): Changed, Fixed, [v0.5.7] - 2026-09-21

### Community 359 - "[v0.0.5] - 2026-07-29"
Cohesion: 0.67
Nodes (3): Added, Fixed, [v0.0.5] - 2026-07-29

### Community 360 - "com.unity.modules.jsonserialize"
Cohesion: 0.08
Nodes (28): com.unity.modules.subsystems, dependencies, depth, source, version, dependencies, depth, source (+20 more)

### Community 361 - "[v0.5.5_1] - 2026-09-18"
Cohesion: 0.67
Nodes (3): Added, Fixed, [v0.5.5_1] - 2026-09-18

### Community 362 - "GainMana"
Cohesion: 0.50
Nodes (3): GameObject, int, GainMana

### Community 363 - "[v0.4.6] - 2026-09-05"
Cohesion: 0.67
Nodes (3): Added, Fixed, [v0.4.6] - 2026-09-05

### Community 365 - "⚠️ [v0.6.3] - 2026-09-23"
Cohesion: 0.67
Nodes (3): Changed, Performance, ⚠️ [v0.6.3] - 2026-09-23

### Community 368 - "[v0.5.0_3] - 2026-09-11"
Cohesion: 0.67
Nodes (3): Added, Fixed, [v0.5.0_3] - 2026-09-11

### Community 369 - "[v0.0.13] - 2026-07-31"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, [v0.0.13] - 2026-07-31

### Community 375 - "StatusEffectCooldownUI"
Cohesion: 0.17
Nodes (10): StatusEffectCooldownUI, float, Image, int, IStatProvider, offset, PointerEventData, subtitle (+2 more)

### Community 376 - "[v0.3.7] - 2026-08-31"
Cohesion: 0.67
Nodes (3): Changed, Fixed, [v0.3.7] - 2026-08-31

### Community 377 - "[v0.0.1] - 2026-07-29 — Initial Release"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.0.1] - 2026-07-29 — Initial Release

### Community 378 - "[v0.2.8] - 2026-08-26"
Cohesion: 0.67
Nodes (3): Removed, Updated, [v0.2.8] - 2026-08-26

### Community 380 - "[v0.5.6] - 2026-09-18"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.5.6] - 2026-09-18

### Community 381 - "[v0.4.11] - 2026-09-10"
Cohesion: 0.67
Nodes (3): Added, Rebalance, [v0.4.11] - 2026-09-10

### Community 382 - "[v0.4.8_1] - 2026-09-05"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.4.8_1] - 2026-09-05

### Community 383 - "[v0.4.5] - 2026-09-04 — Golem Update"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.4.5] - 2026-09-04 — Golem Update

### Community 391 - "[v0.5.0_2] - 2026-09-11"
Cohesion: 0.67
Nodes (3): Changed, Fixed, [v0.5.0_2] - 2026-09-11

### Community 395 - "⚠️ [v0.2.16] - 2026-08-27"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, ⚠️ [v0.2.16] - 2026-08-27

### Community 396 - ".GetExtraEffects"
Cohesion: 0.50
Nodes (3): IAttackEffectSource, AttackType, IReadOnlyList

### Community 399 - "PlayerAttackHandler"
Cohesion: 0.08
Nodes (19): PlayerAttackHandler, QueuedAttack, Animator, bool, Dictionary, float, GameObject, HashSet (+11 more)

### Community 402 - "[v0.1.12_1] - 2026-08-24"
Cohesion: 0.67
Nodes (3): Added, Rebalance, [v0.1.12_1] - 2026-08-24

### Community 404 - "[v0.4.3_2] - 2026-09-03"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.4.3_2] - 2026-09-03

### Community 406 - "[v0.0.23] - 2026-08-06"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.0.23] - 2026-08-06

### Community 407 - "Updated"
Cohesion: 0.67
Nodes (3): Note, Updated, [v0.1.7_2] - 2026-08-18

### Community 465 - "Thorns"
Cohesion: 0.22
Nodes (6): Thorns, bool, Color, float, GameObject, DamageType

### Community 466 - "SkillTreeOpenButton"
Cohesion: 0.12
Nodes (13): SkillTreeOpenButton, bool, Button, GameObject, ICurrencyHolder, ISkillPointHolder, offset, PointerEventData (+5 more)

### Community 498 - "[v0.3.13] - 2026-09-01 - Charged Attacks Update"
Cohesion: 0.50
Nodes (4): Added, Changed, Fixed, [v0.3.13] - 2026-09-01 - Charged Attacks Update

### Community 499 - "CrystalFlux.EntitySystem"
Cohesion: 0.12
Nodes (3): CrystalFlux.Core, CrystalFlux.EntitySystem, CrystalFlux.ProjectileSystem

### Community 521 - "[v0.6.6] - 2026-09-25"
Cohesion: 0.33
Nodes (6): Added, Balance, Buffs, Changed, Nerfs, [v0.6.6] - 2026-09-25

### Community 523 - "[v0.0.22] - 2026-08-06"
Cohesion: 0.67
Nodes (3): Added, Changed, [v0.0.22] - 2026-08-06

### Community 524 - "[v0.0.2] - 2026-07-29"
Cohesion: 0.67
Nodes (3): Added, Fixed, [v0.0.2] - 2026-07-29

### Community 525 - "[v0.5.0_4] - 2026-09-11"
Cohesion: 0.67
Nodes (3): Added, Fixed, [v0.5.0_4] - 2026-09-11

### Community 537 - "WaveManager"
Cohesion: 0.07
Nodes (14): RegularWaveButtonController, WaveManager, bool, Coroutine, DifficultyData, float, int, IStatProvider (+6 more)

## Knowledge Gaps
- **941 isolated node(s):** `ProjectilePattern`, `Added`, `Changed`, `Buffs`, `Nerfs` (+936 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **232 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Projectile` connect `Projectile` to `.Release`, `.CalculateStatGains`, `.HandlePatternMovement`, `EntityProjectileHandler`, `Collectible`, `PlayerUpgradeManager`, `EntitySummonHandler`, `IOrbitRegister`, `IChargeRegister`, `CrystalFlux.EntitySystem`, `.Setup`, `.HandleMovement`, `.HandleHitEntity`, `CombatFeedback`, `MonoBehaviour`?**
  _High betweenness centrality (0.047) - this node is a cross-community bridge._
- **Why does `CrystalFlux.Core` connect `CrystalFlux.Core` to `GameObject`, `GameController`, `EnemyStatManager`, `WaveReward.cs`, `EntityGearManager`, `TextIndicatorSpawner`, `UnlockEffect`, `TextIndicator`, `PrefabPool`, `EntityStatManager`, `UnityEngine.UI`, `CrystalFlux.EntitySystem`, `EntityStats`, `PlayerLevel.cs`, `CooldownAdvance`, `MirageStatManager`, `IPoolable`, `AnomalyInstance`?**
  _High betweenness centrality (0.044) - this node is a cross-community bridge._
- **Why does `WaveManager` connect `WaveManager` to `GameController`, `.GenerateMixedPool`, `.GetCurrentWave`, `PlayerUpgradeManager`, `UnlimitedWaveManager`, `UnityEngine.UI`, `PlayerAttackHandler`, `EntityStatManager`, `.ResumeGameLoop`, `PlayerSkillTree`, `AnomalyInstance`, `RestartConfirmPanelUI`, `MonoBehaviour`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **What connects `ProjectilePattern`, `Added`, `Changed` to the rest of the system?**
  _941 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Anamnesis.slnx` be split into smaller, more focused modules?**
  _Cohesion score 0.019801980198019802 - nodes in this community are weakly interconnected._
- **Should `Projectile` be split into smaller, more focused modules?**
  _Cohesion score 0.0960591133004926 - nodes in this community are weakly interconnected._
- **Should `@PlayerControls` be split into smaller, more focused modules?**
  _Cohesion score 0.06397306397306397 - nodes in this community are weakly interconnected._