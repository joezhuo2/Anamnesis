...
### Pre [v0.4.0]
- [v0.3.9] **Boss rebalance & UI polish**: the three bosses retuned — Cultist and Jellyfish attack values and projectile multipliers reworked, and the Lich dropped to a 2s global cooldown with longer per-attack cooldowns and stronger hits, plus an `EnemyPhase` at 40% HP granting +40% `moveSpeedPct` and +20% `attackSpeedPct`. New `GrantStatusEffect` upgrade type applies an authored `StatusEffect` to the player on any trigger and strips it on removal; new `HoverScale` component drives hover feedback on the reward, anomaly, cooldown and skill-node prefabs and 113 scene objects; and `showCompletionMessage` makes the between-wave subtitle and its 1.5s pause optional. Blizzard now applies the previously unreferenced `Slow 6 15 5`, and the button prefabs moved into `Assets/data/prefabs/Buttons/`
