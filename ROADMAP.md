...
- [v0.4.3] **Sacred Surge & input buffering**: a new armor-scaling rare-pool Skill that cleanses debuffs and can be held to sustain its field; attacks pressed during a cast or charge are now queued and fired on release (`maxQueuedAttacks`, `queueExpiry`); floating numbers abbreviate to k/M and moved to a `TextType` enum; `enableExtraSpawns` lets unlimited waves spawn one enemy per tick. Rebalances: Supernova, unlimited-wave rarity weights
- [v0.4.2] **Overhealth & attack chains**: overhealth — health held above `EffMaxHp`, spent before health and decaying over time — plus the `overhealth` and `healingPct` stats (Core v0.9.0). Four new treasure-pool upgrades: Exsanguinate (healing at full health becomes overhealth), Terminal Cascade (an `additionalAttack` chain can retrigger the attack that started it), Cresendo and Tempo (cooldown advances, the latter on the new dash target)
- [v0.4.1] **Settings menu**: an `Escape`-opened pause panel with live gameplay toggles (enemy health bars, XP drops, gold drops, damage numbers, wave completion message) and full keyboard rebinding, persisted to `settings.json` by the new `CrystalFlux.Settings` assembly. Consolidated the three disjoint `PlayerControls` instances onto one shared, refcounted `GameInput`, without which rebinding could not have worked
- [v0.4.0_1] **Skill tree expansion & balance pass**: 12 new nodes take the tree to 137 — an Attack III tier (+3 Attack, +2% `atkPct`) on both tails of the attack branch and a six-node movement-speed chain off `atk3b`; Solar Wind buffed to 8s, Cyclone Cleave reworked to physical + true damage, Blaze spaced; enemy HP regen scaling cut to a third, +2 armor per level
